...
An optional local LLM generates the replies.
...
| `Program.cs` | ASP.NET host: `/offer` (WebRTC signalling), `/say`, `/ask`. |
| `wwwroot/index.html` | Browser client: connect, plus text boxes to make the avatar speak. |
...
and the avatar should appear and greet you. Use the **Say** box to speak text
verbatim, or the **Ask** box to route a prompt through the LLM first.
