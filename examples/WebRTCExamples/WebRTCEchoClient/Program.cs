                        ...
                        if (_form.Handle != IntPtr.Zero)
                        {
                            unsafe
                            {
                                fixed (byte* s = sample)
                                {
                                    ...
                                    _sourceVideoPicBox.Image?.Dispose();     // Otherwise a GDI handle leaks for every frame.
                        ...
                        if (_form.Handle != IntPtr.Zero)
                        {
                            unsafe
                            {
                                fixed (byte* s = bmp)
                                {
                                    ...
                                    _echoVideoPicBox.Image?.Dispose();       // Otherwise a GDI handle leaks for every frame.
