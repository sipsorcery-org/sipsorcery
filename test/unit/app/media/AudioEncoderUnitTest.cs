...
namespace SIPSorcery.Media.UnitTests
{
    ...
    public class AudioEncoderUnitTest
    {
        ...
        public void OpusEncodePreservesDurationUnitTest()
        {
            ...
            var encoded = encoder.EncodeAudio(pcm, AudioCommonlyUsedFormats.OpusWebRTC);
            ...
            var samplesPerChannel = independentDecoder.Decode(encoded, decodedPcm, decodedPcm.Length, false);
        ...
        public void OpusDecodeReturnsMonoPcmUnitTest()
        {
            ...
            var encoded = encoder.EncodeAudio(pcm, AudioCommonlyUsedFormats.OpusWebRTC);
            var decoded = decoder.DecodeAudio(encoded, AudioCommonlyUsedFormats.OpusWebRTC);
        ...
        public void OpusRoundTripPreservesPitchUnitTest()
        {
            ...
            var inputCrossings = ZeroCrossings(i => pcm[i], pcm.Length);
            var outputCrossings = ZeroCrossings(i => decodedPcm[i], samplesPerChannel);
        ...
        public void OpusDecodeOfStereoStreamDownmixesToMonoUnitTest()
        {
            ...
            var encodedLength = stereoEncoder.Encode(stereoPcm, SAMPLES_PER_FRAME, packet, packet.Length);
            Assert.True(encodedLength > 0, "The stereo OPUS encode produced an empty packet.");

            var decoder = new AudioEncoder(includeOpus: true);
            var decoded = decoder.DecodeAudio(packet.AsSpan(0, encodedLength).ToArray(), AudioCommonlyUsedFormats.OpusWebRTC);
