	...
	public class Crypto
	{
		...
		public static int GetRandomInt(int start, int end)
		{
            if (m_randomNumber == null)
            {
                lock (typeof(Crypto))
                {
                    int randomSeed = Convert.ToInt32(DateTime.Now.Ticks % Int32.MaxValue);
		...
		public static string GetSHAHash(string plainText)
		{
			...
			return Convert.ToBase64String(shaHash.ComputeHash(Encoding.UTF8.GetBytes(plainText)));
		}

        public static string GetSHAHashAsHex(string plainText)
        {
            SHA1Managed shaHash = new SHA1Managed();
            byte[] hash = shaHash.ComputeHash(Encoding.UTF8.GetBytes(plainText));
