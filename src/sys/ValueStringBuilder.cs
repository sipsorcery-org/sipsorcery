...
internal ref partial struct ValueStringBuilder
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
#if NET6_0_OR_GREATER
    public void Append(ulong value, string? format = null, IFormatProvider? provider = null) => AppendSpanFormattable(value, format, provider);
#else
    public void Append(ulong value, string? format = null, IFormatProvider? provider = null) => Append(value.ToString(format, provider));
#endif
    ...
    {
        if (additionalCapacityBeyondPos == 0)
        {
            additionalCapacityBeyondPos = 2 * Capacity - _pos; // Default to double the current capacity minus the current position.
        }
