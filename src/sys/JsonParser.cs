...
//
// Description: A very simple JSON deserialiser. Intended for cases where a fully
// fledged JSON deserialiser is not supported, for example issues were encountered
// with the Unity game runtime with Newtonsoft and System.Text implementations.
//
// Based on https://github.com/zanders3/json/blob/master/src/JSONParser.cs.
//
// History:
            ...
            if (type == typeof(string))
            {
                ...
                for (int i = 1; i < json.Length - 1; ++i)
                {
                    if (json[i] == '\\' && i + 1 < json.Length - 1)
                    {
                        ...
                        {
                            uint c = 0;
                            if (UInt32.TryParse(json.AsSpan(i + 2, 4), System.Globalization.NumberStyles.AllowHexSpecifier, null, out c))
                            {
