                        ...
                        while (!IsClosed)
                        {
                            try
                            {
                                ...
                                if (bytesRead > 0)
                                {
                                    ...
                                    ProcessRawBuffer(m_pipe.Reader, (IPEndPoint)Socket.LocalEndPoint, (IPEndPoint)Socket.RemoteEndPoint);
                                }
                            }
            ...
            try
            {
                ...
                while (reader.TryRead(out var readResult) && readResult.Buffer.Length > STUNHeader.STUN_HEADER_LENGTH)
                {
                    ...
                    static ReadOnlyMemory<byte> ReadBuffer(ReadOnlySequence<byte> buffer, ref byte[]? rentedBuffer)
                    {
                        ...
                        if (rentedBuffer is not null && rentedBuffer.Length < buffer.Length)
                        {
