...
namespace SIPSorcery.SIP
{
    ...
    public class SIPTCPChannel : SIPChannel
    {
            ...
            bool useDualMode) : base(sipEncoding,sipBodyEncoding)
        {
            ...
            logger.LogTcpChannelCreated(ProtDescr, ListeningSIPEndPoint);
        ...
        {
            logger.LogTcpSocketConnectionStart(ProtDescr, ListeningSIPEndPoint);
            ...
            while (!Closed)
            {
                try
                {
                    ...
                    if (!Closed)
                    {
                        ...
                        logger.LogTcpSocketConnectionAccepted(ProtDescr, remoteEndPoint, ListeningSIPEndPoint);
                ...
                catch (Exception acceptExcp)
                {
                    ...
                    logger.LogTcpSocketConnectionError(ProtDescr, acceptExcp.GetType(), acceptExcp.Message, acceptExcp);
                }
            }
        }
        ...
        private void ProcessReceive(SocketAsyncEventArgs e)
        {
            ...
            catch (Exception excp)
            {
                ...
                logger.LogTcpStreamReceiveError(ProtDescr, e.RemoteEndPoint, excp.Message, excp);
        ...
        private void ProcessSend(SocketAsyncEventArgs e)
        {
            ...
            if (e.BytesTransferred == 0 || e.SocketError != SocketError.Success)
            {
                ...
                logger.LogTcpSocketSendError(ProtDescr, e.RemoteEndPoint, e.SocketError);
                OnSIPStreamDisconnected(streamConn, e.SocketError);
            }
        }
        ...
        internal async Task<SocketError> ConnectClientAsync(IPEndPoint dstEndPoint, byte[] buffer, string serverCertificateName)
        {
            try
            {
                ...
                logger.LogTcpSocketConnectClientEndpoint(ProtDescr, ListeningSIPEndPoint, dstEndPoint);
                ...
                logger.LogTcpSocketAttemptingConnection(ListeningSIPEndPoint, dstEndPoint);
                ...
                {
                    logger.LogTcpDestinationError(ProtDescr, dstEndPoint, connectTcs.Task.Result);
                    return SocketError.TimedOut;
                }
                else if (connectTcs.Task.Result != SocketError.Success)
                {
                    logger.LogTcpSendError(ProtDescr, dstEndPoint, connectTcs.Task.Result);
                    return connectTcs.Task.Result;
                }
                else
                {
                    logger.LogTcpSocketConnectCompleted(ProtDescr, ListeningSIPEndPoint, dstEndPoint, connectTcs.Task.Result);
                    ...
                    {
                        logger.LogTcpConnectFailure(ProtDescr, dstEndPoint, dstEndPoint, postConnectResult);
                    }
                    else
                    {
                        m_connections.TryAdd(sipStmConn.ConnectionID, sipStmConn);
            ...
            {
                logger.LogConnectClientError(excp.Message, excp);
            ...
            string connectionIDHint)
        {
            try
            {
                ...
                {
                    logger.LogTcpSendLocalBlockError(ProtDescr, dstSIPEndPoint, SIPConstants.DEFAULT_ENCODING.GetString(buffer));
                ...
                else
                {
                    ...
                    {
                        logger.LogTcpConnectionInitiateError(ProtDescr, dstSIPEndPoint);
            ...
            {
                logger.LogTcpChannelSendError(dstSIPEndPoint, excp.Message, excp);
                throw;
            }
        }
        ...
        protected virtual Task SendOnConnected(SIPStreamConnection sipStreamConn, byte[] buffer)
        {
            ...
            {
                logger.LogTcpSocketSendException(ProtDescr, sipStreamConn.RemoteSIPEndPoint, sockExcp.SocketErrorCode, sockExcp.Message, sockExcp);
        ...
        protected void OnSIPStreamDisconnected(SIPStreamConnection connection, SocketError socketError)
        {
            try
            {
                if (connection != null)
                {
                    if (socketError == SocketError.ConnectionReset)
                    {
                        ...
                        logger.LogTcpStreamDisconnected(ProtDescr, connection.RemoteSIPEndPoint, socketError);
                    }
                    else
                    {
                        logger.LogTcpStreamDisconnectionWarning(ProtDescr, connection.RemoteSIPEndPoint, socketError);
                    }
            ...
            {
                logger.LogTcpConnectionError(excp.Message, excp);
            }
        }
        ...
        public override void Close()
        {
            if (!Closed == true)
            {
                logger.LogTcpSocketClosing(ProtDescr, ListeningEndPoint);
                ...
                lock (m_connections)
                {
                    foreach (SIPStreamConnection streamConnection in m_connections.Values)
                    {
                        ...
                        {
                            logger.LogTcpChannelClose(ProtDescr, closeExcp.Message, closeExcp);
                        }
                    }
                    m_connections.Clear();
                }
                ...
                if (m_channelSocket != null)
                {
                    try
                    {
                        logger.LogTcpSocketStoppingListener(ProtDescr, ListeningEndPoint);

                        m_channelSocket.Close();
                    }
                    catch (Exception stopExcp)
                    {
                        logger.LogTcpSocketListenerError(ProtDescr, stopExcp.Message, stopExcp);
                    }
                }

                logger.LogTcpSocketCloseSuccess(ProtDescr, ListeningEndPoint);
            }
        }
        ...
        private void PruneConnections()
        {
            try
            {
                ...
                while (!Closed)
                {
                    ...
                    while (!checkComplete)
                    {
                        try
                        {
                            ...
                            {
                                logger.LogTcpSocketPruneInactive(ProtDescr, ListeningSIPEndPoint, inactiveConnection.RemoteSIPEndPoint);
                        ...
                        catch (SocketException sockExcp)
                        {
                            ...
                            logger.LogTcpSocketPruningError(sockExcp.Message, sockExcp.ErrorCode, sockExcp);
                        }
                        catch (Exception pruneExcp)
                        {
                            logger.LogTcpSocketPruningMessage(pruneExcp.Message, pruneExcp);
                            checkComplete = true;
                        }
                    }
                ...
                logger.LogTcpSocketPruningHalted(ProtDescr, ListeningSIPEndPoint);
            }
            catch (OperationCanceledException) { }
            catch (AggregateException) { } // This gets thrown if task is cancelled.
            catch (Exception excp)
            {
                logger.LogTcpSocketPruningException(ProtDescr, excp.Message, excp);
            }
        }
    }
}
