    ...
    public class WindowsAudioEndPoint : IAudioEndPoint, IDisposable
    {
        ...
        private void InitCaptureDevice(int audioInDeviceIndex, int audioSourceSampleRate, int audioSourceChannels)
        {
            if (WaveIn.DeviceCount > 0)
            {
                ...
                if (audioInDeviceIndex >= -1 && audioInDeviceIndex < WaveIn.DeviceCount)
                {
                    try
                    {
                ...
                {
                    logger.LogAudioInputDeviceIndexExceeded(audioInDeviceIndex, WaveIn.DeviceCount - 1);
                    OnAudioSourceError?.Invoke($"The requested audio input device index {audioInDeviceIndex} exceeds the maximum index of {WaveIn.DeviceCount - 1}. Use -1 for default device.");
                }
            }
            else
            {
