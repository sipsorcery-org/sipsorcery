...
namespace SIPSorcery.Media
{
    public class RtpAudioSession : RTPSession, IMediaSession
    {
        ...
        public void SendMedia(SDPMediaTypesEnum mediaType, uint samplePeriod, byte[] sample)
        {
            ...
            {
                // TODO: Don't want to be parsing the payloadID for every RTP packet, store it somewhere.
                var sdp = SDP.ParseSDPDescription(localDescription.sdp);
                int payloadID = Convert.ToInt32(sdp.Media.Where(x => x.Media == SDPMediaTypesEnum.audio).First().MediaFormats.First().FormatID);
