...
namespace SIPSorcery.Media
{
    public class AudioEncoder : IAudioEncoder, IDisposable
    {
        ...
        public AudioEncoder(bool includeLinearFormats = false, bool includeOpus = false)
        {
            ...
            if (includeOpus)
            {
                _supportedFormats.Add(new AudioFormat(111, nameof(AudioCodecsEnum.OPUS), OPUS_SAMPLE_RATE, OPUS_CHANNELS, "useinbandfec=1"));
            }
        }
        ...
        public byte[] EncodeAudio(short[] pcm, AudioFormat format)
        {
            ...
            EncodeAudio(pcm, format, buffer);
            return buffer.GetReadOnlySequence().ToArray();
        }
        ...
        public void EncodeAudio(ReadOnlySpan<short> pcm, AudioFormat format, IBufferWriter<byte> destination)
        {
            ...
            else if (format.Codec == AudioCodecsEnum.G729)
            {
                ...
                MemoryMarshal.AsBytes(pcm).CopyTo(pcmBytes);
                var encoded = _g729Encoder.Process(pcmBytes);
            ...
            else if (format.Codec == AudioCodecsEnum.OPUS)
            {
                ...
                else
                {
                    ...
                    destination.Write(encodedSample.Slice(0, encodedLength));
                }
            }
            else
            {
        ...
        {
            var buffer = new PooledBuffer<short>();
            DecodeAudio(encodedSample.AsSpan(), format, buffer);
            return buffer.GetReadOnlySequence().ToArray();
        }
        ...
        public void DecodeAudio(ReadOnlySpan<byte> encodedSample, AudioFormat format, IBufferWriter<short> destination)
        {
            if (format.Codec == AudioCodecsEnum.G722)
            {
                ...
                var estimatedOutputSize = encodedSample.Length * 2;
                
                // Convert ReadOnlySpan to byte[] since G722Codec.Decode requires byte[]
                var encodedArray = encodedSample.ToArray();
                var tempDecoded = new short[estimatedOutputSize];
                var actualSampleCount = _g722Decoder.Decode(_g722DecoderState, tempDecoded, encodedArray, encodedArray.Length);
                
                // Copy the decoded samples to the output
                var outputSpan = destination.GetSpan(actualSampleCount);
                tempDecoded.AsSpan(0, actualSampleCount).CopyTo(outputSpan);
                destination.Advance(actualSampleCount);
            }
            else if (format.Codec == AudioCodecsEnum.G729)
            {
                ...
                // Convert span to byte array for G729Decoder (it requires byte[])
                var encodedArray = encodedSample.ToArray();
                var decodedBytes = _g729Decoder.Process(encodedArray);
                var decodedShorts = MemoryMarshal.Cast<byte, short>(decodedBytes.AsSpan());
                var outputSpan = destination.GetSpan(decodedShorts.Length);
                decodedShorts.CopyTo(outputSpan);
                destination.Advance(decodedShorts.Length);
            }
            else if (format.Codec == AudioCodecsEnum.PCMA)
            {
            ...
            else if (format.Codec == AudioCodecsEnum.OPUS)
            {
                ...
                // Convert span to byte array for Opus decoder (it requires byte[])
                var encodedArray = encodedSample.ToArray();
                ...
                var totalFloats = samplesPerChannel * _opusDecoder.NumChannels;

                // Convert to 16-bit interleaved PCM.
                var outputSpan = destination.GetSpan(totalFloats);
                for (var i = 0; i < totalFloats; i++)
                {
                    var f = ClampToFloat(floatBuf[i], -1.0f, 1.0f);
                    outputSpan[i] = (short)(f * 32767);
                }
                destination.Advance(totalFloats);
            }
            else
            {
