...
namespace SIPSorceryMedia.FFmpeg
{
    ...
    /// <summary>
    /// Interface for an encoder that can have its bitrate adjusted.
    /// </summary>
    public interface IEncoder
    {
        ...
        /// </summary>
        void Tune(int BitRate, int Framerate);
    }
    ...
    public class TWCCBitrateController
    {
        ...
        public TWCCBitrateController(IEncoder encoder)
        {
            _encoder = encoder;
        ...
        public void ProcessFeedback(RTCPTWCCFeedback feedback)
        {
            ...
            lock (_lock)
            {
                ...
                if (elapsedMs >= UpdateIntervalMs)
                {
                    ...
                    _encoder.Tune((int)_currentBitrate, (int)_framerate);
        ...
        public void CalculateMaxBitrate(int width, int height, int framerate, AVCodecID codecId)
        {
            ...
            {
                if (width * height <= 352 * 288)        // CIF or smaller
                    baseBitratePerFrame = 13;
                else if (width * height <= 640 * 480)   // VGA
                    baseBitratePerFrame = 35;
                else if (width * height <= 1280 * 720)  // 720p
                    baseBitratePerFrame = 87;
                else if (width * height <= 1920 * 1080) // 1080p
