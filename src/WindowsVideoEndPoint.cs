...
namespace SIPSorceryMedia.Windows
{
    ...
    public class WindowsVideoEndPoint : IVideoSource, IVideoSink, IDisposable
    {
        ...
        private const string MF_NV12_PIXEL_FORMAT = "NV12";
        ...
        private async void FrameArrivedHandler(MediaFrameReader sender, MediaFrameArrivedEventArgs e)
        {
            if (!_isClosed)
            {
                ...
                {
                    if (_isClosed || frame == null || frame.VideoMediaFrame == null) return;
        ...
        private async Task<bool> InitialiseDevice(uint width, uint height, uint fps)
        {
            ...
            logger.LogInformation($"Video capture device successfully initialised: {_width}x{_height} {fpsSelected:0.##}fps pixel format {_mediaFrameSource.CurrentFormat.Subtype}.");
