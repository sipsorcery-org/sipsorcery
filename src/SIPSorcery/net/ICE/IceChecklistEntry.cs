...
public enum ChecklistEntryState
{
    ...
    /// <summary>
    /// A check has been sent for this pair, and it failed (a response to the check was never received, or a failure
    /// response was received).
    /// </summary>
    Failed,

    /// <summary>
    /// A check for this pair has not been sent, and it cannot be sent until the pair is unfrozen and moved into the
    /// Waiting state.
    /// </summary>
    Frozen
}
...
internal enum ChecklistState
{
    ...
    /// <summary>
    /// The checklist does not have a valid pair for each component of the data stream, and all of the candidate pairs
    /// in the checklist are in either the Failed or the Succeeded state. In other words, at least one component of the
    /// checklist has candidate pairs that are all in the Failed state, which means the component has failed, which
    /// means the checklist has failed.
    /// </summary>
    Failed
}

/// <summary>
/// A check list entry represents an ICE candidate pair (local candidate + remote candidate) that is being checked for
/// connectivity. If the overall ICE session does succeed it will be due to one of these checklist entries successfully
/// completing the ICE checks.
/// </summary>
public class ChecklistEntry : IComparable
{
    ...
    /// <summary>
    /// The current state of this checklist entry. Indicates whether a STUN check has been sent, responded to, timed out
    /// etc.
    /// </summary>
    /// <remarks>
    /// See https://tools.ietf.org/html/rfc8445#section-6.1.2.6 for the state transition diagram for a check list entry.
    /// </remarks>
    public ChecklistEntryState State = ChecklistEntryState.Frozen;

    /// <summary>
    /// The candidate pairs whose local and remote candidates are both the default candidates for a particular component
    /// is called the "default candidate pair" for that component. This is the pair that would be used to transmit data
    /// if both agents had not been ICE aware.
    /// </summary>
    public bool Default;

    /// <summary>
    /// Gets set to true when the connectivity checks for the candidate pair are successful. Valid entries are eligible
    /// to be set as nominated.
    /// </summary>
    public bool Valid;

    /// <summary>
    /// Gets set to true if this entry is selected as the single nominated entry to be used for the session
    /// communications. Setting a check list entry as nominated indicates the ICE checks have been successful and the
    /// application can begin normal communications.
    /// </summary>
    ...
    /// <summary>
    /// The priority for the candidate pair: - Let G be the priority for the candidate provided by the controlling
    /// agent. - Let D be the priority for the candidate provided by the controlled agent. Pair Priority = 2^32*MIN(G,D)
    /// + 2*MAX(G,D) + (G>D?1:0)
    /// </summary>
    /// <remarks>
    ...
    public string? RequestTransactionID
    {
        ...
        set
        {
            if (value is { })
            {
                ...
                if (value != currentValue && _cachedRequestTransactionIDs is { })
                {
    ...
    /// <summary>
    /// Before a remote peer will be able to use the relay it's IP address needs to be authorised by sending a Create
    /// Permissions request to the TURN server. This field records the number of Create Permissions requests that have
    /// been sent for this entry.
    /// </summary>
    ...
    {
        if (other is ChecklistEntry checklistEntry)
        {
            return checklistEntry.Priority.CompareTo(Priority);
        }
        else
        {
            throw new SipSorceryException("CompareTo is not implemented for ChecklistEntry and arbitrary types.");
        }
    }
    ...
    internal void GotStunResponse(STUNMessage stunResponse, IPEndPoint remoteEndPoint)
    {
            ...
            if (stunResponse.Attributes is { })
            {
                foreach (var attr in stunResponse.Attributes)
                {
                    if (attr.AttributeType == STUNAttributeTypesEnum.ErrorCode)
                    {
                        ...
                        {
                            if (LocalCandidate.IceServer is null)
                            {
        ...
        switch (stunResponse.Header.MessageType)
        {
                    ...
                    foreach (var attr in stunResponse.Attributes)
                    {
                        if (attr.AttributeType == STUNAttributeTypesEnum.Lifetime)
                        {
                            ...
                            LocalCandidate.IceServer.TurnTimeToExpiry =
                                DateTime.Now + TimeSpan.FromSeconds(BinaryPrimitives.ReadUInt32BigEndian(attr.Value.Span));
                            break;
                        }
                    }
                }
            ...
            case STUNMessageTypesEnum.CreatePermissionErrorResponse:
                Debug.Assert(stunResponse.Attributes is not null);
                var errCodeAttribute = default(STUNErrorCodeAttribute);
                foreach (var attr in stunResponse.Attributes)
                {
                    if (attr.AttributeType == STUNAttributeTypesEnum.ErrorCode)
                    {
                        errCodeAttribute = attr as STUNErrorCodeAttribute;
                        break;
                    }
                }
                ...
                if (retry)
                {
                    ...
                    logger.LogIceTurnResendCreatePermission(errCodeAttribute?.ErrorCode, remoteEndPoint, RemoteCandidate?.DestinationEndPoint);
                }
                else
                {
                    ...
                    if (errCodeAttribute is not null)
                    {
                        logger.LogIceTurnCreatePermissionError(errCodeAttribute.ErrorCode, errCodeAttribute.ReasonPhrase, remoteEndPoint, RemoteCandidate?.DestinationEndPoint);
                    }
                    else
                    {
                        logger.LogIceTurnCreatePermissionError(remoteEndPoint, RemoteCandidate?.DestinationEndPoint);
                    }
                    TurnPermissionsResponseAt = DateTime.Now;
