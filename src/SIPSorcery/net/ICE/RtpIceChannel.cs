...
/// <summary>
/// An RTP ICE Channel carries out connectivity checks with a remote peer in an attempt to determine the best
/// destination end point to communicate with the remote party.
/// </summary>
/// <remarks>
/// Local server reflexive candidates don't get added to the checklist since they are just local "host" candidates with
/// an extra NAT address mapping. The NAT address mapping is needed for the remote ICE peer but locally a server
/// reflexive candidate is always going to be represented by a "host" candidate. Limitations: - To reduce complexity
/// only a single checklist is used. This is based on the main webrtc use case where RTP (audio and video) and RTCP are
/// all multiplexed on a single socket pair. Therefore there only needs to be a single component and single data stream.
/// If an additional use case occurs then multiple checklists could be added. Developer Notes: There are 4 main tasks
/// occurring during the ICE checks: - Local candidates: ICE server checks (which can take seconds) are being carried
/// out to gather "server reflexive" and "relay" candidates. - Remote candidates: the remote peer should be trickling in
/// its candidates which need to be validated and if accepted new entries added to the checklist. - Checklist
/// connectivity checks: the candidate pairs in the checklist need to have connectivity checks sent. - Match STUN
/// messages: STUN requests and responses are being received and need to be matched to either an ICE server check or a
/// checklist entry check. After matching action needs to be taken to update the status of the ICE server or checklist
/// entry check.
/// </remarks>
public partial class RtpIceChannel : RTPChannel
{
    ...
    /// <summary>
    /// True if we are the "controlling" ICE agent (we initiated the communications) or false if we are the "controlled"
    /// agent.
    /// </summary>
    public bool IsController { get; internal set; }

    /// <summary>
    /// Optional hook to normalize the source endpoint of received STUN binding requests before they're matched against
    /// the remote candidate list / checklist. Used to reconcile hairpin scenarios where a peer reaches this agent via a
    /// TURN relay running on the same machine — the observed source IP is a local interface address but the remote
    /// candidate was advertised with the relay's public IP. The delegate returns the translated endpoint, or <c>
    /// null</c> / the input unchanged when no translation applies.
    /// <see cref="RTCPeerConnection.RemoteEndpointTranslator"/> forwards its value here.
    /// </summary>
    ...
    /// <summary>
    /// A queue of remote ICE candidates that have been added to the session and that are waiting to be processed to
    /// determine if they will create a new checklist entry.
    /// </summary>
    ...
    /// <summary>
    /// For local candidates this implementation takes a shortcut to reduce complexity. The RTP socket will always be
    /// bound to one of: - IPAddress.IPv6Any [::], - IPAddress.Any 0.0.0.0, or, - a specific single IP address. As such
    /// it's only necessary to create a single checklist entry to cover all local Host type candidates. Host candidates
    /// must still be generated, based on all local IP addresses, and will need to be transmitted to the remote peer but
    /// they don't need to be used when populating the checklist.
    /// </summary>
    internal readonly RTCIceCandidate _localChecklistCandidate;

    /// <summary>
    /// If a TURN server is being used for this session and has received a successful response to the allocate request
    /// then this field will hold the candidate to use in the checklist.
    /// </summary>
    internal RTCIceCandidate? _relayChecklistCandidate;

    /// <summary>
    /// If the connectivity checks are successful this will hold the entry that was nominated by the connection check
    /// process.
    /// </summary>
    ...
    /// <summary>
    /// This event gets fired when a STUN message is sent by this channel. The event is for diagnostic purposes only.
    /// Parameters: - STUNMessage: The STUN message that was sent. - IPEndPoint: The remote end point the STUN message
    /// was sent to. - bool: True if the message was sent via a TURN server relay.
    /// </summary>
    ...
    /// </summary>
    /// <remarks>
    /// The order is <see cref="MdnsGetAddresses"/>, then <see cref="MdnsResolve"/>. If both are null system
    /// <see cref="Dns">DNS resolver</see> will be used.
    /// </remarks>
    ...
    /// Optional. If this is not set then the default is to bind to the IPv6 wildcard address in dual mode to the IPv4
    /// wildcard address if IPv6 is not available.
    /// </param>
    /// <param name="component">
    /// The component (RTP or RTCP) the channel is being used for. Note for cases where RTP and RTCP are multiplexed the
    ...
    /// <param name="includeAllInterfaceAddresses">
    /// If set to true then IP addresses from ALL local interfaces will be used for host ICE candidates. If left as the
    /// default false value host candidates will be restricted to the single interface that the OS routing table matches
    /// to the destination address or the Internet facing interface if the destination is not known. The restrictive
    /// behaviour is as per the recommendation at:
            ...
            foreach (var (uri, iceServer) in resolvedIceServers)
            {
                switch (iceServer.Protocol)
                {
                    case ProtocolType.Udp:
                        Debug.Assert(RtpConnection is not null);
                        iceServerConnections[uri] = RtpConnection;
                        break;
                    case ProtocolType.Tcp when iceServer._uri.Scheme is STUNSchemesEnum.turn:
                        {
                    ...
                    case ProtocolType.Tcp when iceServer._uri.Scheme is STUNSchemesEnum.turns:
                        {
                            NetServices.CreateRtpSocket(
                                false,
                                ProtocolType.Tcp,
                            ...
                            Debug.Assert(rtpTcpSocket is { });
                            var iceServerConnection = new SocketTlsConnection(rtpTcpSocket, iceServer._uri.Host, iceServer.SslClientAuthenticationOptions);
                            iceServerConnection.OnPacketReceived += OnRTPPacketReceived;
                            iceServerConnection.OnClosed += reason => CloseIceServerTcpConnection(iceServerConnection, reason);
                            iceServerConnections[uri] = iceServerConnection;
                            break;
    ...
    /// <summary>
    /// We've been given the green light to start the ICE candidate gathering process. This could include contacting
    /// external STUN and TURN servers. Events will be fired as each ICE is identified and as the gathering state
        ...
        if (!_closed && IceGatheringState == RTCIceGatheringState.@new)
        {
            ...
            OnIceGatheringStateChange?.Invoke(IceGatheringState);
                ...
                if (_policy == RTCIceTransportPolicy.all)
                {
                    ...
                    logger.LogIceLocalCandidateCount(_candidates.Count);
                }
                else if(_policy == RTCIceTransportPolicy.relay)
                {
                    logger.LogIceRelayOnlyPolicy();
                }
    ...
    /// <summary>
    /// Set the ICE credentials that have been supplied by the remote peer. Once these are set the connectivity checks
    /// should be able to commence.
    /// </summary>
    ...
    /// <summary>
    /// Acquires an ICE candidate for each IP address that this host has except for: - Loopback addresses must not be
    /// included. - Deprecated IPv4-compatible IPv6 addresses and IPv6 site-local unicast addresses must not be
    /// included, - IPv4-mapped IPv6 address should not be included. - If a non-location tracking IPv6 address is
    /// available use it and do not included location tracking enabled IPv6 addresses (i.e. prefer temporary IPv6
    /// addresses over permanent addresses), see RFC6724. SECURITY NOTE:
    /// https://tools.ietf.org/html/draft-ietf-rtcweb-ip-handling-12#section-5.2 Makes recommendations about how host IP
    /// address information should be exposed. Of particular relevance are: Mode 1: Enumerate all addresses: WebRTC MUST
    /// use all network interfaces to attempt communication with STUN servers, TURN servers, or peers.This will converge
    /// on the best media path, and is ideal when media performance is the highest priority, but it discloses the most
    /// information. Mode 2: Default route + associated local addresses: WebRTC MUST follow the kernel routing table
    /// rules, which will typically cause media packets to take the same route as the application's HTTP traffic. If an
    /// enterprise TURN server is present, the preferred route MUST be through this TURN server.Once an interface has
    /// been chosen, the private IPv4 and IPv6 addresses associated with this interface MUST be discovered and provided
    /// to the application as host candidates.This ensures that direct connections can still be established in this
    /// mode. This implementation implements Mode 2.
    /// </summary>
    /// <remarks>
    /// See https://tools.ietf.org/html/rfc8445#section-5.1.1.1 See https://tools.ietf.org/html/rfc6874 for a
    /// recommendation on how scope or zone ID's should be represented as strings in IPv6 link local addresses. Due to
    /// parsing issues in at least two other WebRTC stacks (as of Feb 2021) any zone ID is removed from an ICE candidate
    /// string.
    /// </remarks>
    ...
    private void RefreshTurn(object? state)
    {
        try
        {
            ...
            {
                logger.LogIceTurnRefreshRequest(_activeIceServer._uri);
            ...
            {
                logger.LogIceTurnPermissionsFailed(NominatedEntry.LocalCandidate.IceServer._uri, NominatedEntry.TurnPermissionsRequestSent);
            }
                     ...
                     REFRESH_PERMISSION_PERIOD)
            {
                ...
                logger.LogIceTurnPermissionsRequest(turnPermissionsRequestSent, iceServer._uri, destinationEndPoint, requestTransactionID);
    ...
    /// <summary>
    /// Checks the list of ICE servers to perform STUN binding or TURN reservation requests. Only one of the ICE server
    /// entries should end up being used. If at least one TURN server is provided it will take precedence as it can
    /// potentially supply both Server Reflexive and Relay candidates.
    /// </summary>
    private void CheckIceServers(object? state)
    {
        ...
        if (Monitor.TryEnter(_iceServerResolver))
        {
            try
            {
                if (_activeIceServer is null || _activeIceServer.Error != SocketError.Success)
                {
                    ...
                    foreach (var (uri, iceServer) in _iceServerResolver.IceServers)
                    {
                        ...
                        if (selectedIceServer is null
                            || iceServer._uri.Scheme > selectedIceServer._uri.Scheme)
                        {
                ...
                var activeIceServerUri = activeIceServer._uri;
    ...
    /// <summary>
    /// Adds candidates and updates the checklist for an ICE server that has completed the initial connectivity checks.
    /// </summary>
    /// <param name="iceServer">
    /// The ICE server that the initial checks have been completed for.
    /// </param>
    private async Task AddCandidatesForIceServer(IceServer iceServer)
    {
        ...
        if (iceServer.ServerReflexiveEndPoint is { })
        {
            ...
            {
                logger.LogIcePeerReflexAddingCandidate(iceServer._uri, iceServer.ServerReflexiveEndPoint);
        ...
        if (_relayChecklistCandidate is null && iceServer.RelayEndPoint is { })
        {
            ...
            {
                logger.LogIceRelayAddingCandidate(iceServer._uri, iceServer.RelayEndPoint);
    ...
    /// </summary>
    /// <remarks>
    /// From https://tools.ietf.org/html/rfc8445#section-6.1.2.2: IPv6 link-local addresses MUST NOT be paired with
    ...
    /// The remote candidate to attempt to create a new checklist entry for.
    /// </param>
    private async Task UpdateChecklist(RTCIceCandidate localCandidate, RTCIceCandidate remoteCandidate)
    {
        ...
        try
        {
            ...
            if (remoteCandidate.DestinationEndPoint is not null)
            {
    ...
    /// <summary>
    /// Attempts to add a checklist entry. If there is already an equivalent entry in the checklist the entry may not be
    /// added or may replace an existing entry.
    /// </summary>
                        ...
                        else
                        {
                            ...
                            if (IceGatheringState == RTCIceGatheringState.complete)
                            {
                                ...
                                if (_checklist.TrueForAll(static x => x.State == ChecklistEntryState.Failed))
                                {
                                    ...
                                    logger.LogIceChecklistEntriesFailed();
    ...
    /// <summary>
    /// Sets the nominated checklist entry. This action completes the checklist processing and indicates the connection
    ...
    /// If true indicates we are acting as the "controlling" ICE agent and are nominating this candidate as the chosen
    /// one.
    /// </param>
    /// <remarks>
    /// As specified in https://tools.ietf.org/html/rfc8445#section-7.2.4. Relay candidates are a special (and more
    /// difficult) case. The extra steps required to send packets via a TURN server are: - A Channel Bind request needs
    /// to be sent for each peer end point the channel will be used to communicate with. - Packets need to be sent and
    /// received as TURN Channel Data messages.
    /// </remarks>
    private void SendConnectivityCheck(ChecklistEntry candidatePair, bool setUseCandidate)
    {
        ...
        if (isRelayCheck && candidatePair.TurnPermissionsResponseAt == DateTime.MinValue)
        {
            ...
            {
                logger.LogIceTurnPermissionsFailed(localCandidate.IceServer?._uri, candidatePair.TurnPermissionsRequestSent);
    ...
    /// The candidate pair identifying the remote peer to send the STUN Binding Request to.
    /// </param>
    ...
    /// <summary>
    /// Builds and sends the connectivity check on a candidate pair that is set as the current nominated, connected
    /// pair.
    /// </summary>
    ...
    /// </summary>
    /// <remarks>
    /// Actions to take on a successful STUN response https://tools.ietf.org/html/rfc8445#section-7.2.5.3 - Discover
    /// peer reflexive remote candidates as per https://tools.ietf.org/html/rfc8445#section-7.2.5.3.1. - Construct a
    /// valid pair which means match a candidate pair in the check list and mark it as valid (since a successful STUN
    /// exchange has now taken place on it). A new entry may need to be created for this pair for a peer reflexive
    /// candidate. - Update state of candidate pair that generated the check to Succeeded. - If the controlling
    /// candidate set the USE_CANDIDATE attribute then the ICE agent that receives the successful response sets the
    /// nominated flag of the pair to true. Once the nominated flag is set it concludes the ICE processing for that
    /// component.
    /// </remarks>
    ...
    /// True of the request was relayed via the TURN server in use by this ICE channel (i.e. the ICE server that this
    /// channel is acting as the client with).
    /// </param>
    private void GotStunBindingRequest(STUNMessage bindingRequest, IPEndPoint remoteEndPoint, bool wasRelayed)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                lock (_checklistLock)
                {
                    ...
                    foreach (var entry in _checklist)
                    {
                        if ((entry.RemoteCandidate.IsEquivalentEndPoint(RTCIceProtocol.udp, remoteEndPoint)
                                || entry.RemoteCandidate.IsEquivalentEndPoint(RTCIceProtocol.udp, canonicalEndPoint))
                ...
                bool IsUnknownRemoteCandidate(IPEndPoint remoteEndPoint, IPEndPoint canonicalEndPoint)
                {
                    foreach (var candidate in _remoteCandidates)
                    {
                        if (candidate.IsEquivalentEndPoint(RTCIceProtocol.udp, remoteEndPoint)
                            || candidate.IsEquivalentEndPoint(RTCIceProtocol.udp, canonicalEndPoint))
                        {
                            return false;
                        }
                    }
                ...
                if (matchingChecklistEntry is null
                    && ((_remoteCandidates is null) || IsUnknownRemoteCandidate(remoteEndPoint, canonicalEndPoint)))
                {
                ...
                else if (matchingChecklistEntry == null && !ReferenceEquals(canonicalEndPoint, remoteEndPoint))
                {
                    ...
                    RTCIceCandidate GetCanonicalCandidateOrThrow(IPEndPoint endPoint)
                    {
                        foreach (var candidate in _remoteCandidates)
                        {
                            if (candidate.IsEquivalentEndPoint(RTCIceProtocol.udp, endPoint))
                            {
                                return candidate;
                            }
                        }

                        throw new InvalidOperationException("No matching canonical remote candidate was found.");
                    }

                    var canonicalCandidate = GetCanonicalCandidateOrThrow(canonicalEndPoint);

                    ChecklistEntry? entry = null;
                    lock (_checklistLock)
                    {
                        foreach (var checklistEntry in _checklist)
                        {
                            if (ReferenceEquals(checklistEntry.RemoteCandidate, canonicalCandidate)
                                && (!wasRelayed || checklistEntry.LocalCandidate.type == RTCIceCandidateType.relay))
                            {
                    ...
                    if (entry == null)
                    {
                        var localCandidate = wasRelayed ? _relayChecklistCandidate : _localChecklistCandidate;
                        Debug.Assert(localCandidate is { });
                        entry = new ChecklistEntry(localCandidate, canonicalCandidate, IsController);
                        entry.State = ChecklistEntryState.Waiting;
                        if (wasRelayed)
                        {
                            entry.TurnPermissionsRequestSent = 1;
    ...
    /// <summary>
    /// Checks a STUN response transaction ID to determine if it matches a check being carried out for an ICE server.
    /// </summary>
    ...
    /// <returns>
    /// The result of the send attempt. Note this is the return code from the socket send call and not the result code
    ...
    private SocketError SendStunBindingRequest(IceServer iceServer)
    {
        ...
        var iceServerUri = iceServer._uri;
    ...
    /// The transaction ID to set on the request. This gets used to match responses back to the sender.
    /// </param>
    /// <param name="iceServer">The ICE server to send the request to.</param>
    ...
    {
        if (!m_iceServerConnections.TryGetValue(iceServer._uri, out var iceServerConnection))
        {
            throw new InvalidOperationException($"Invalid ICE server: {iceServer._uri}");
        }
    ...
    private async Task<IPAddress[]> ResolveMdnsName(RTCIceCandidate candidate)
    {
        ...
        if (addresses.Length == 0)
        {
            logger.LogMdnsResolutionNoAnswers(candidate.address);
    ...
    /// <summary>
    /// The send method for the RTP ICE channel. The sole purpose of this overload is to package up sends that need to
    /// be relayed via a TURN server. If the connected channel is not a relay then the send can be passed straight
    /// through to the underlying RTP channel.
    /// </summary>
    ...
    /// <returns>
    /// The result of initiating the send. This result does not reflect anything about whether the remote party received
    /// the packet or not.
    /// </returns>
