...
/// <summary>
/// If ICE servers (STUN or TURN) are being used with the session this class is used to track the connection state for
/// each server that gets used.
/// </summary>
public class IceServer
{
    ...
    /// <summary>
    /// A magic cookie to use as the prefix for STUN requests generated for ICE servers. Allows quick matching of
    /// responses for ICE servers compared to responses for ICE candidate connectivity checks;
    /// </summary>
    internal const string ICE_SERVER_TXID_PREFIX = "91245";

    /// <summary>
    /// The length of the magic cookie of server ID that are used as the prefix for each ICE server transaction ID.
    /// </summary>
    ...
    /// <summary>
    /// The maximum ICE server ID that can be set. Means the number of ICE servers per session is limited to 10.
    /// Checking 10 ICE servers when attempting to establish a peer connection seems very, very high. It would generally
    /// be expected that only 1 or 2 ICE servers would ever be used.
    /// </summary>
    internal const int MAXIMUM_ICE_SERVER_ID = 9;

    /// <summary>
    /// The maximum number of requests to send to an ICE server without getting a response.
    /// </summary>
    internal const int MAX_REQUESTS = 25;

    /// <summary>
    /// The maximum number of error responses before failing the ICE server checks. A success response will reset the
    /// count.
    /// </summary>
    ...
    /// <summary>
    /// The period at which to refresh a successful STUN binding. If the ICE server did not get used as the nominated
    /// candidate the ICE server checks timer will be stopped.
    /// </summary>
    ...
    /// <summary>
    /// The end point for this STUN or TURN server. Will be set asynchronously once any required DNS lookup completes.
    /// </summary>
    public IPEndPoint? ServerEndPoint { get; set; }

    /// <summary>
    /// The transaction ID to use in STUN requests. It is used to match responses with connection checks for this ICE
    /// serve entry.
    /// </summary>
    ...
    /// <summary>
    /// The number of requests that have been sent to the server without a response.
    /// </summary>
    ...
    /// <summary>
    /// Records the failure message if there was an error configuring or contacting the STUN or TURN server.
    /// </summary>
    internal SocketError Error { get; set; } = SocketError.Success;

    /// <summary>
    /// If the initial Binding (for STUN) or Allocate (for TURN) connection check is successful this will hold the
    /// resultant server reflexive transport address.
    /// </summary>
    public IPEndPoint? ServerReflexiveEndPoint { get; set; }

    /// <summary>
    /// If the ICE server being checked is a TURN one and the Allocate request is successful this will hold the relay
    /// transport address.
    /// </summary>
    internal IPEndPoint? RelayEndPoint { get; set; }

    /// <summary>
    /// If requests to the server need to be authenticated this is the nonce to set. Normally the nonce will come from
    /// the server in a 401 Unauthorized response.
    /// </summary>
    internal ReadOnlyMemory<byte> Nonce { get; set; }

    /// <summary>
    /// If requests to the server need to be authenticated this is the realm to set. The realm may be known in advance
    /// or can come from the server in a 401 Unauthorized response.
    /// </summary>
    ...
    /// <param name="id">
    /// Needs to be set uniquely for each ICE server used in this session. Gets added to the transaction ID to
    /// facilitate quick matching of STUN requests and responses. Needs to be between 0 and 9.
    /// </param>
    ...
    /// <summary>
    /// Parses a semicolon-delimited ICE server span into an IceServer instance. Expected format:
    /// urls[;username[;credential]] Examples: "stun:stun.example.com:3478"
    /// "turn:turn.example.com?transport=tcp;user1;pass1" "stun:stun1.example.com,stun:stun2.example.com" Notes: -
    /// Whitespace is trimmed. - Surrounding quotes are removed from fields. - If multiple URLs are provided in the
    /// first field (comma or whitespace separated), the first non-empty URL is used. - If the URL lacks a scheme (e.g.
    /// "example.com:3478"), a stun: scheme will be assumed.
    /// </summary>
    ...
    /// <summary>
    /// Gets an ICE candidate for this ICE server once the required server responses have been received. Note the
    /// related address and port are deliberately not set to avoid leaking information about internal network
    /// configuration.
    /// </summary>
    ...
    /// <summary>
    /// Checks whether a STUN response transaction ID belongs to a request that was sent for this ICE server entry.
    /// </summary>
    ...
    /// <summary>
    /// Handler for a STUN response received in response to an ICE server connectivity check. Note that no STUN requests
    /// are expected to be received from an ICE server during the initial connection to an ICE server. Requests will
    /// only arrive if a TURN relay is used and data indications arrive but this will be at a later stage.
    /// </summary>
    ...
    /// <returns>
    /// True if the STUN response resulted in new ICE candidates being available (which will be either a "server
    /// reflexive" or "relay" candidate.
    /// </returns>
    ...
    /// <summary>
    /// Handles a 401 Unauthorized / 438 Stale Nonce challenge to an authenticated request (Allocate, Binding or
    /// Refresh): captures the nonce and realm and rotates the transaction ID so the request can be retried with
    /// credentials. The first, unauthenticated challenge is the expected trigger for authentication and is discounted
    /// from the error budget; a challenge that comes back AFTER credentials were already sent (indicated by a non-null
