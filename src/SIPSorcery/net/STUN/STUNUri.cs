...
public sealed class STUNUri : IEquatable<STUNUri>
{
    ...
    public static bool TryParse(ReadOnlySpan<char> uriSpan, [NotNullWhen(true)] out STUNUri? uri)
    {
        ...
        if (scheme is STUNSchemesEnum.stuns or STUNSchemesEnum.turns)
        {
    ...
    {
        return obj is STUNUri other && Equals(this, other);
    }
    ...
    public override int GetHashCode()
    {
        ...
        return HashCode.Combine(
            Scheme,
            Transport,
            Host == null ? 0 : StringComparer.OrdinalIgnoreCase.GetHashCode(Host),
            Port,
            ExplicitPort);
    }
}
