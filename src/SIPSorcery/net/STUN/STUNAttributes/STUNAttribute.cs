...
public partial class STUNAttribute : IByteSerializable
{
    ...
    public static ReadOnlySpan<byte> ParseMessageAttributes(ReadOnlySpan<byte> buffer, STUNHeader? header, List<STUNAttribute> attributes)
    {
        while (buffer.Length >= 4)
        {
            ...
            var stunAttributeValueLength = stunAttributeValue?.Length ?? 0;
            if (stunAttributeValueLength < GetMinimumValueLength(attributeType))
            {
                logger.LogStunAttributeValueLengthShort(attributeType, stunAttributeValueLength);
            }
            else
            {
    ...
    private static int GetMinimumValueLength(STUNAttributeTypesEnum attributeType)
        ...
        {
            STUNAttributeTypesEnum.MappedAddress or STUNAttributeTypesEnum.AlternateServer or STUNAttributeTypesEnum.XORMappedAddress or STUNAttributeTypesEnum.XORPeerAddress or STUNAttributeTypesEnum.XORRelayedAddress
            ...
            STUNAttributeTypesEnum.ChangeRequest or STUNAttributeTypesEnum.ErrorCode or STUNAttributeTypesEnum.ConnectionId
                => SINGLE_WORD_VALUE_LENGTH,
            _ => 0,
        };
