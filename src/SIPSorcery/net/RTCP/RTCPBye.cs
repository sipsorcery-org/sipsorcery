...
public partial class RTCPBye : IByteSerializable
{
    ...
    {
        var reasonLength = (Reason is { }) ? Encoding.UTF8.GetByteCount(Reason) : 0;
        var paddedLength = GetPaddedLength(reasonLength);
        ...
        if (reasonLength > 0)
        {
            var reasonByteCount = paddedLength - sizeof(uint);
            var buffer = writer.GetSpan(reasonByteCount);
            buffer[0] = (byte)reasonLength;
            Encoding.UTF8.GetBytes(Reason.AsSpan(), buffer.Slice(1));

            if (paddedLength > reasonByteCount)
            {
                buffer.Slice(reasonLength + 1, reasonByteCount - reasonLength - 1).Clear();
            }

            writer.Advance(reasonByteCount);
        }
