...
public partial class RTCPFeedback : IByteSerializable
{
    ...
    public byte BitrateExp; // Bitrate Expoent
    public uint BitrateMantissa; //Bits per Second
    public uint FeedbackSSRC //Packet sender
    {
        get => FeedbackSSRCs is { Length: > 0 } ? FeedbackSSRCs[0] : 0;
        ...
        internal set
        {
            ...
            Debug.Assert(FeedbackSSRCs is not null);
    ...
    /// <summary>
    /// Constructor for RTP feedback reports that do not require any additional feedback control indication parameters
    /// (e.g. RTCP Rapid Resynchronisation Request).
    /// </summary>
    ...
    /// <summary>
    /// Constructor for payload feedback reports that do not require any additional feedback control indication
    /// parameters (e.g. Picture Loss Indication reports).
    /// </summary>
    /// <param name="feedbackMessageType">The payload specific feedback type.</param>
    ...
    public RTCPFeedback(ReadOnlySpan<byte> packet)
    {
        ...
        switch (Header)
        {
            case { PacketType: RTCPReportTypesEnum.RTPFB, FeedbackMessageType: RTCPFeedbackTypesEnum.RTCP_SR_REQ }:
                SENDER_PAYLOAD_SIZE = 8;
                // PLI feedback reports do no have any additional parameters.
                break;
            case { PacketType: RTCPReportTypesEnum.RTPFB }:
                SENDER_PAYLOAD_SIZE = 12;
            ...
            case { PacketType: RTCPReportTypesEnum.PSFB, PayloadFeedbackMessageType: PSFBFeedbackTypesEnum.PLI }:
                SENDER_PAYLOAD_SIZE = 8;
                break;
            ...
            case { PacketType: RTCPReportTypesEnum.PSFB, PayloadFeedbackMessageType: PSFBFeedbackTypesEnum.AFB }:
    ...
    /// <inheritdoc/>
    public int GetByteCount() => RTCPHeader.HEADER_BYTES_LENGTH + SENDER_PAYLOAD_SIZE;
    ...
    private void WriteBytesCore(Span<byte> buffer)
    {
        ...
        using var writer = new ArrayPoolBufferWriter<byte>(RTCPHeader.HEADER_BYTES_LENGTH);
        ...
        switch (Header)
        {
            ...
            case { PacketType: RTCPReportTypesEnum.PSFB, PayloadFeedbackMessageType: PSFBFeedbackTypesEnum.AFB }:
                ...
                //Fix UniqueID Size
                Span<char> uniqueIdBytes = stackalloc char[4];
                uniqueIdBytes.Clear();
                if (UniqueID is not null)
                {
                    var length = Math.Min(4, UniqueID.Length);
                    UniqueID.AsSpan(0, length).CopyTo(uniqueIdBytes);
                }

                var currentCounter = RTCPHeader.HEADER_BYTES_LENGTH + 8;
                Encoding.ASCII.GetBytes(uniqueIdBytes, buffer.Slice(currentCounter));
                currentCounter += 4;
                ...
                if (string.Equals(UniqueID, "REMB", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    buffer[currentCounter + 2] = (byte)temp;
                    buffer[currentCounter + 1] = (byte)(temp >>= 8);
                    buffer[currentCounter] = (byte)((BitrateExp << 2) | (byte)((temp >>= 8) & 0b11));
                    currentCounter += 3;

                    for (var i = 0; i < FeedbackSSRCs.Length; i++)
                    {
                        BinaryPrimitives.WriteUInt32BigEndian(buffer.Slice(currentCounter), FeedbackSSRCs[i]);
                ...
                break;
            default:
                logger?.LogRtcpFeedbackSerializationNotImplemented(Header.PacketType, Header.FeedbackMessageType);
                break;
