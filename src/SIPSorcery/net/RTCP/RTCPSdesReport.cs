...
/// Only the mandatory CNAME item is supported.
/// </summary>
public partial class RTCPSDesReport : IByteSerializable
{
    ...
    /// <param name="cname">
    /// Canonical End-Point Identifier SDES item. This should be a unique string common to all RTP streams in use by the
    /// application. Maximum length is 255 bytes (note bytes not characters).
    /// </param>
    public RTCPSDesReport(uint ssrc, string cname)
    {
        ArgumentNullException.ThrowIfNullOrEmpty(cname);
    ...
    public RTCPSDesReport(ReadOnlySpan<byte> packet)
    {
        ...
        // {
        //     throw new SipSorceryException("The packet did not contain the minimum number of bytes for an RTCP SDES packet.");
        // }
        // else if (packet[8] != CNAME_ID)
        // {
        //     throw new SipSorceryException("The RTCP report packet did not have the required CNAME type field set correctly.");
        // }
    ...
    {
        Debug.Assert(CNAME is { });
        return RTCPHeader.HEADER_BYTES_LENGTH + GetPaddedLength(Encoding.UTF8.GetByteCount(CNAME));
    }
    ...
    {
        Debug.Assert(CNAME is { });
        var cnameLength = Encoding.UTF8.GetByteCount(CNAME);
        var paddedLength = GetPaddedLength(cnameLength);
        var size = RTCPHeader.HEADER_BYTES_LENGTH + paddedLength;
        Debug.Assert(size == GetByteCount());
        ...
        var cNameByteCount = Encoding.UTF8.GetBytes(CNAME.AsSpan(), buffer.Slice(RTCPHeader.HEADER_BYTES_LENGTH + 6));
        buffer[RTCPHeader.HEADER_BYTES_LENGTH + 5] = (byte)cNameByteCount;

        var length = cNameByteCount + sizeof(int) + sizeof(byte) + sizeof(byte);
        if (paddedLength > length)
        {
            buffer.Slice(length + 1 + RTCPHeader.HEADER_BYTES_LENGTH, paddedLength - length - 1).Clear();
        }

        writer.Advance(paddedLength);
