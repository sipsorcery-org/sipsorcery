...
public class SctpDataSender
{
    ...
    internal uint _outstandingBytes
    {
        get
        {
            ...
            {
                Debug.Assert(kvp.Value.UserData is { });
                total += (uint)kvp.Value.UserData.Length;
            }
            return total;
        }
    }
    ...
    public ulong BufferedAmount
    {
        ...
        {
            ulong total = 0;
            foreach (var x in _sendQueue)
            {
                total += (ulong)(x.UserData?.Length ?? 0);
            }
            return total;
        }
    }
        ...
        if (sack is { })
        {
            ...
            unchecked
            {
                ...
                if (_unconfirmedChunks.TryGetValue(sack.CumulativeTsnAck, out var result))
                {
                    ...
                    Debug.Assert(result.UserData is { });
    ...
    public void SendData(ushort streamID, uint ppid, byte[] data, int offset = 0, int count = -1)
    {
        ...
        lock (_sendQueue)
        {
            ...
            if (_streamSeqnums.TryGetValue(streamID, out var streamSeqnum))
            {
                unchecked
                {
                    _streamSeqnums[streamID] = (ushort)(streamSeqnum + 1);
            ...
            for (int index = 0; index * _defaultMTU < dataCount; index++)
            {
                ...
                bool isEnd = (chunkOffset + payloadLength) >= dataCount;
    ...
    private void ProcessGapReports(List<SctpTsnGapBlock> sackGapBlocks, uint maxTSNDistance, bool didSackIncrementTSN)
    {
        ...
        unchecked
        {
            ...
            foreach (var gapBlock in sackGapBlocks)
            {
                ...
                {
                    logger.LogSctpSackGapReportStartTooDistant(goodTSNStart, lastAckTSN);
                    break;
                }
                else if (!SctpDataReceiver.IsNewer(lastAckTSN, goodTSNStart))
                {
                    logger.LogSctpSackGapReportStartBehind(goodTSNStart, lastAckTSN);
                    break;
                }
                else
                {
                    ...
                    while (missingTSN != goodTSNStart)
                    {
                        if (!_missingChunks.TryGetValue(missingTSN, out int missCount))
                        {
                            if (!_unconfirmedChunks.ContainsKey(missingTSN))
                            {
                                ...
                                logger.LogSctpNoMatchingUnconfirmedChunk(missingTSN);
                                break;
                            }
                            else
                            {
                            ...
                            SctpDataReceiver.IsNewer(missingTSN, highestTsnNewlyAcknowledged))
                        {
                            ...
                            if (missCount + 1 == 3)
                            {
                                if (!_inFastRecoveryMode) // RFC4960 7.2.4 (2)
                                {
                                    ...
                                    Debug.Assert(sackGapBlocks.Count > 0);
                                    _fastRecoveryExitPoint = _cumulativeAckTSN + sackGapBlocks[^1].End;
    ...
    {
        logger.LogSctpDataSendThreadStarted(_associationID);
        ...
        while (!_isClosed)
        {
            ...
            var burstSize = (_inRetransmitMode || _inFastRecoveryMode || _congestionWindow < outstandingBytes || _receiverWindow == 0) ? 1 : MAX_BURST;
            var chunksSent = 0;
                ...
                foreach (var missing in _missingChunks)
                {
                    if (missing.Value >= 3)  // RFC4960 7.2.4 Fast retransmission
                    {
                        if (_unconfirmedChunks.TryGetValue(missing.Key, out var missingChunk))
                        {
                            ...
                            Debug.Assert(missingChunk.UserData is { });
            ...
            {
                var retransmitNeeded = burstSize - chunksSent;
                var retransmitSent = 0;
                foreach (var chunk in _unconfirmedChunks.Values)
                {
                    var elapsed = now.Subtract(chunk.LastSentAt).TotalMilliseconds;
                    var rto = _hasRoundTripTime ? _rto : _rtoInitialMilliseconds;
                    ...
                    if (elapsed > rto)
                    {
                        ...
                        Debug.Assert(chunk.UserData is { });
                        ...
                        if (retransmitSent >= retransmitNeeded)
                        {
                            break;
                        }
                    }
            ...
            if (chunksSent < burstSize && _sendQueue.Count > 0 && _congestionWindow > outstandingBytes)
            {
                while (chunksSent < burstSize && _sendQueue.TryDequeue(out var dataChunk))
                {
                    ...
                    Debug.Assert(dataChunk.UserData is { });
        ...
        logger.LogSctpDataSendThreadStopped(_associationID);
    }
