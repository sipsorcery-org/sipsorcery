...
public partial class SctpPacket : IByteSerializable
{
    ...
    private static (List<SctpChunk> chunks, List<byte[]> unrecognisedChunks) ParseChunks(ReadOnlySpan<byte> buffer)
    {
        ...
        while (posn < buffer.Length)
        {
            ...
            {
                throw new SipSorceryException("The SCTP packet buffer was too short to contain a complete chunk header.");
            }

            var chunkSpan = buffer.Slice(posn);
            var chunkType = chunkSpan[0];

            uint chunkLength = SctpChunk.GetChunkLengthFromHeader(chunkSpan, false);
            if (chunkLength < SctpChunk.SCTP_CHUNK_HEADER_LENGTH)
            {
                throw new SipSorceryException($"The SCTP chunk length was invalid. The minimum length is {SctpChunk.SCTP_CHUNK_HEADER_LENGTH} bytes but the packet specified {chunkLength} bytes.");
            }

            if (posn + chunkLength > buffer.Length)
            {
                throw new SipSorceryException($"The SCTP packet buffer was too short. Required {chunkLength} chunk bytes but only {buffer.Length - posn} available.");
            }
