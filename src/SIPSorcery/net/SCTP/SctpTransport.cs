...
public abstract partial class SctpTransport
{
    ...
    protected SctpPacket GetInitAck(SctpPacket initPacket, IPEndPoint? remoteEP)
    {
        ...
        {
            var result = hmac.ComputeHash(buffer.WrittenMemory);
        ...
        initAckChunk.StateCookie = buffer.WrittenMemory.ToArray();
    ...
    protected string GetCookieHMAC(byte[] buffer)
    {
        ...
        using var tempBuffer = new ArrayPoolBufferWriter<byte>(0);
        ...
        using (var hmac = new HMACSHA256(_hmacKey))
        {
            var result = hmac.ComputeHash(tempBuffer.WrittenMemory);
