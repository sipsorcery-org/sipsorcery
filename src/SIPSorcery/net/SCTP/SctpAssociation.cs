...
public class SctpAssociation
{
    ...
    public uint TSN
    {
        get
        {
            Debug.Assert(_dataSender is { });
            return _dataSender.TSN;
        }
    }
    ...
    public void UpdateSourcePort(ushort port)
    {
        ...
        {
            logger.LogSctpSourcePortCannotBeUpdated(State);
        }
        else
        {
    ...
    public void UpdateDestinationPort(ushort port)
    {
        if (State != SctpAssociationState.Closed)
        {
            logger.LogSctpDestinationPortCannotBeUpdated(State);
        }
        else
        {
    ...
    public void Init()
    {
        ...
        {
            logger.LogSctpAssociationCannotInitialiseAfterAbortOrShutdown();
        }
        ...
        {
            logger.LogSctpAssociationCannotInitialise(State);
        }
    }
    ...
    public void GotCookie(SctpTransportCookie cookie)
    {
        ...
        {
            throw new SipSorceryException($"SCTP association cannot initialise with a cookie in state {State}.");
        }
        else
        {
            ...
            foreach (var chunk in packet.Chunks)
            {
                ...
                switch (chunkType)
                {
                    case SctpChunkType.ABORT:
                        var sctpAbortChunk = chunk as SctpAbortChunk;
                        Debug.Assert(sctpAbortChunk is not null);
                        var abortReason = sctpAbortChunk.GetAbortReason();
                    ...
                    case SctpChunkType.DATA:
                        ...
                        if (dataChunk.UserData is null || dataChunk.UserData.Length == 0)
                        {
                            // Fatal condition:
                    ...
                    case SctpChunkType.INIT_ACK when State == SctpAssociationState.CookieWait:
                        ...
                        else
                        {
                            ...
                            if (initAckChunk.UnrecognizedPeerParameters.Count > 0)
                            {
                                ...
                                {
                                    var buffer = new byte[unrecognised.GetParameterLength(true)];
                                    _ = unrecognised.WriteBytes(buffer);
                                    var unrecognisedParams = new SctpErrorUnrecognizedParameters { UnrecognizedParameters = buffer };
                    ...
                    case SctpChunkType.INIT_ACK when State != SctpAssociationState.CookieWait:
                        logger.LogSctpInitAckInWrongState(State);
                        break;

                    case SctpChunkType.SACK:
                        Debug.Assert(_dataSender is { });
                        Debug.Assert(chunk is SctpSackChunk);
                    ...
                    case SctpChunkType.SHUTDOWN_ACK when State == SctpAssociationState.ShutdownSent:
                        ...
                        shutCompletePkt.SetHeaderVerificationTag(packet.Header.VerificationTag);
                        SendPacket(shutCompletePkt);
                        break;

                    case SctpChunkType.SHUTDOWN_COMPLETE when (State is SctpAssociationState.ShutdownAckSent or SctpAssociationState.ShutdownSent):
                        _wasShutdown = true;
                        SetState(SctpAssociationState.Closed);
                        break;

                    default:
                        logger.LogSctpAssociationNoRuleForChunk(chunkType, State);
                        break;
                }
            }
        }
    ...
    public void SendData(ushort streamID, uint ppid, byte[] data, int offset = 0, int count = -1)
    {
        if (_wasAborted)
        {
            logger.LogSctpSendDataNotAllowedAfterAbort();
        }
        else if (State is not (SctpAssociationState.Established or
                  SctpAssociationState.ShutdownPending or
                  SctpAssociationState.ShutdownReceived))
        {
            logger.LogSctpAssociationCannotSendDataInState(State);
        }
        else
        {
            Debug.Assert(_dataSender is { });
    ...
    public void Abort(ISctpErrorCause errorCause)
    {
        if (!_wasAborted)
        {
            ...
            OnAborted?.Invoke(errorCause.CauseCode.ToStringFast());
    ...
    private void SendInit()
    {
        if (!_wasAborted)
        {
            ...
            var size = init.GetByteCount();
            var memoryOwner = MemoryPool<byte>.Shared.Rent(size);
            _ = init.WriteBytes(memoryOwner.Memory.Span);

            _sctpTransport.Send(ID, memoryOwner.Memory.Slice(0, size), memoryOwner);
    ...
    internal void SendChunk(SctpChunk chunk)
    {
        if (!_wasAborted)
        {
            ...
            var size = pkt.GetByteCount();
            var memoryOwner = MemoryPool<byte>.Shared.Rent(size);
            _ = pkt.WriteBytes(memoryOwner.Memory.Span);
    ...
    private void T1InitTimerExpired(object? state)
    {
        ...
        {
            Debug.Assert(_t1Init is { });
            _t1Init.Dispose();
            _t1Init = null;
            _initialisationFailed = true;

            logger.LogSctpAssociationTimedOutInitAck();
    ...
    private void T1CookieTimerExpired(object? state)
    {
        ...
        {
            Debug.Assert(_t1Cookie is { });
            _t1Cookie.Dispose();
            _t1Cookie = null;
            _initialisationFailed = true;

            logger.LogSctpAssociationTimedOutCookieAck();
