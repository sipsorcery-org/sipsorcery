...
public partial class SctpChunk
{
    ...
    /// differently.
    /// </summary>
    /// <param name="writer">The writer to write the serialised chunk bytes to. It
    /// must have the required space already allocated.</param>
    /// <returns>The number of bytes, including padding, written to the buffer.</returns>
    public virtual ushort WriteTo(IBufferWriter<byte> writer)
    {
        var byteCount = GetByteCount(true);
        var span = writer.GetSpan(byteCount);

        var bytesWritten = WriteChunkHeader(span);

        if (ChunkValue is { Length: > 0 } chunkValue)
        {
            chunkValue.CopyTo(span.Slice(SCTP_CHUNK_HEADER_LENGTH));
        }
