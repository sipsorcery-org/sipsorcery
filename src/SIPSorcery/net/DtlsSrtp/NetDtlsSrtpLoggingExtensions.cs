...
internal static partial class NetDtlsSrtpLoggingExtensions
{
    ...
    [LoggerMessage(
        ...
        SkipEnabledCheck = true)]
    private static partial void LogDtlsServerCertificateChainReceivedUnchecked(
        this ILogger logger,
        int chainLength);
    ...
    [LoggerMessage(
        ...
        SkipEnabledCheck = true)]
    private static partial void LogDtlsServerCertificateFingerprintUnchecked(
        this ILogger logger,
        string fingerprint,
        string subject);

    public static void LogDtlsServerCertificateChainReceived(
        this ILogger logger,
        Certificate clientCertificate)
    {
        if (logger.IsEnabled(LogLevel.Debug))
        {
            ...
            logger.LogDtlsServerCertificateChainReceivedUnchecked(chain.Length);

            for (int i = 0; i != chain.Length; i++)
            {
                var entry = X509CertificateStructure.GetInstance(chain[i].GetEncoded());
                logger.LogDtlsClientServerCertificateFingerprintUnchecked(DtlsCertificateUtils.Fingerprint(entry), entry.Subject.ToString());
            }
        }
    }
    ...
    [LoggerMessage(
        ...
        SkipEnabledCheck = true)]
    private static partial void LogDtlsClientServerCertificateChainReceivedUnchecked(
        this ILogger logger,
        int chainLength);
    ...
    [LoggerMessage(
        ...
        SkipEnabledCheck = true)]
    private static partial void LogDtlsClientServerCertificateFingerprintUnchecked(
        this ILogger logger,
        string fingerprint,
        string subject);

    public static void LogDtlsClientServerCertificateChainReceived(
        this ILogger logger,
        TlsCertificate[] chain)
    {
        if (logger.IsEnabled(LogLevel.Debug))
        {
            logger.LogDtlsClientServerCertificateChainReceivedUnchecked(chain.Length);
            for (int i = 0; i != chain.Length; i++)
            {
                var entry = X509CertificateStructure.GetInstance(chain[i].GetEncoded());
                logger.LogDtlsClientServerCertificateFingerprintUnchecked(DtlsCertificateUtils.Fingerprint(entry), entry.Subject.ToString());
