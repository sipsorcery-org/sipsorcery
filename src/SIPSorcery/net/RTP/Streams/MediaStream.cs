...
public class MediaStream
{
    ...
    /// <summary>
    /// Tracks the global order in which this stream was added to the session, across all media types. Used to preserve
    /// m-line ordering per RFC 3264 §8.
    /// </summary>
    public int MediaInsertionOrder = -1;

    /// <summary>
    /// Fires when the connection for a media type is classified as timed out due to not receiving any RTP or RTCP
    /// packets within the given period.
    /// </summary>
    ...
    /// <summary>
    /// Gets fired when an RTP packet is received from a remote party. Parameters are: - index of the AudioStream or
    /// VideoStream - Remote endpoint packet was received from, - The media type the packet contains, will be audio or
    /// video, - The full RTP packet.
    /// </summary>
    public event Action<int, IPEndPoint, SDPMediaTypesEnum, RTPPacket>? OnRtpPacketReceivedByIndex;

    /// <summary>
    /// Gets fired when an RTP Header packet is received from a remote party. Parameters are: - index of the AudioStream
    /// or VideoStream - Remote endpoint packet was received from, - The media type the packet contains, will be audio
    /// or video, - The RTP Header exension URI.
    /// </summary>
    ...
    /// <summary>
    /// In order to detect RTP events from the remote party this property needs to be negotiated to a common payload ID.
    /// RTP events are typically DTMF tones.
    /// </summary>
    ...
    /// <summary>
    /// This endpoint is used when a relay server (TURN) is being used for the RTP session. All RTP packets will be sent
    /// to the relay end point instead of the DestinationEndPoint.
    /// </summary>
    public TurnRelayEndPoint? RtpRelayEndPoint { get; set; }

    /// <summary>
    /// This endpoint is used when a relay server (TURN) is being used for the RTCP session. All RTCP packets will be
    /// sent to the relay end point instead of the ControlDestinationEndPoint.
    /// </summary>
    public IPEndPoint? RelayControlDestinationEndPoint { get; set; }

    /// <summary>
    /// If set to true indicates the RTP and RTCP sockets are for a relay server (TURN). All traffic for the session
    /// should then be sent to/from the relay and not updated.
    /// </summary>
    ...
    private (bool, ReadOnlyMemory<byte>) UnprotectBuffer(ReadOnlyMemory<byte> buffer)
    {
        if (SecureContext != null)
        {
            ...
            if (MemoryMarshal.TryGetArray(buffer, out var segment) && segment.Offset == 0 && segment.Array is { })
            {
                res = unprotectRtpPacket(segment.Array, segment.Count, out outBufLen);
            }
            else
            {
                var tempBuf = ArrayPool<byte>.Shared.Rent(buffer.Length);
                try
                {
                    buffer.CopyTo(tempBuf);
                    res = unprotectRtpPacket(tempBuf, buffer.Length, out outBufLen);
                }
                finally
                {
    ...
    protected void SendRtpRaw(ReadOnlySpan<byte> data, uint timestamp, int markerBit, int payloadType, bool checkDone, ushort? seqNum = null)
    {
        if (checkDone || CheckIfCanSendRtpRaw())
        {
            ...
            */
            ...
            {
                using var extensionBuffer = new ArrayPoolBufferWriter<byte>();
    ...
    /// <summary>
    /// Do any additional processing for the RTP packet. For vidoe streams this method will be overridden to handle
    /// video packetisation. Audio and other media types typially don't use framing but have other processing they'd
    /// like to do.
    /// </summary>
    ...
    /// <returns>
    /// True if remote end point for this media type was the expected one or it was adjusted. False if the remote end
    /// point was deemed to be invalid for this media type.
    /// </returns>
    ...
    /// </summary>
    /// <returns>
    /// A new RTCPSession object. The RTCPSession must have its Start method called in order to commence sending RTCP
    /// reports.
    /// </returns>
