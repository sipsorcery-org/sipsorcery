...
public class H264Packetiser
{
    ...
    public static IEnumerable<H264Nal> ParseNals(ReadOnlySpan<byte> accessUnit)
    {
        var result = new List<H264Nal>();
        int zeroes = 0;
        ...
        for (int i = 0; i < accessUnit.Length; i++)
        {
            ...
            else if (accessUnit[i] == 0x01 && zeroes >= 2)
            {
                ...
                if (nalStart - currPosn > 4)
                {
                    ...
                    result.Add(new H264Nal(accessUnit.Slice(currPosn, nalSize).ToArray(), isLast));
                }
        ...
        {
            result.Add(new H264Nal(accessUnit.Slice(currPosn).ToArray(), true));
        }
