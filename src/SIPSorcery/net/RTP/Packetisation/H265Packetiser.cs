...
public class H265Packetiser
{
    ...
    public static IEnumerable<H265Nal> ParseNals(ReadOnlySpan<byte> accessUnit)
    {
        var result = new List<H265Nal>();
        int zeroes = 0;
        ...
        for (int i = 0; i < accessUnit.Length; i++)
        {
            ...
            else if (accessUnit[i] == 0x01 && zeroes >= 2)
            {
                ...
                if (nalStart - currPosn > 4)
                {
                    ...
                    result.Add(new H265Nal(accessUnit.Slice(currPosn, nalSize).ToArray(), isLast));
                }
        ...
        {
            result.Add(new H265Nal(accessUnit.Slice(currPosn).ToArray(), true));
        }
