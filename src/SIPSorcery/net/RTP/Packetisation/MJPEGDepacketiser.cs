...
public class MJPEGDepacketiser
{
    ...
    private ArrayPoolBufferWriter<byte> _frameBuffer = new ArrayPoolBufferWriter<byte>(0);
    ...
    private void GenerateFrame(IBufferWriter<byte> returnFrame)
    {
        ...
        {
            _frameBuffer.Write(EndMarkerBytes);
        }

        returnFrame.Write(_frameBuffer.WrittenSpan);

        _frameBuffer.Clear();

        static bool EndsWithEndMarkerBytes(ArrayPoolBufferWriter<byte> buffer)
        {
            if (buffer.WrittenCount < EndMarkerBytes.Length)
            {
                return false;
            }

            return buffer.WrittenSpan[^EndMarkerBytes.Length..].SequenceEqual(EndMarkerBytes);
        }
    }
}
