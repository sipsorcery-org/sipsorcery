...
public class RtpVideoFramer
{
    ...
    {
        if (codec is not (VideoCodecsEnum.VP8 or VideoCodecsEnum.VP9 or VideoCodecsEnum.AV1 or VideoCodecsEnum.H264 or VideoCodecsEnum.H265 or VideoCodecsEnum.JPEG))
        {
    ...
    public bool GotRtpPacket(IBufferWriter<byte> bufferWriter, RTPPacket rtpPacket)
    {
        ...
        switch (_codec)
        {
            ...
            case VideoCodecsEnum.VP9:
                {
                    Debug.Assert(_vp9Depacketiser is { });
                    if (_vp9Depacketiser.ProcessRTPPayload(bufferWriter, payload, hdr.SequenceNumber, hdr.Timestamp, hdr.MarkerBit, out var isKeyFrame))
                    {
                        return true;
                    }
                }
            ...
            case VideoCodecsEnum.AV1:
                {
                    Debug.Assert(_av1Depacketiser is { });
                    if (_av1Depacketiser.ProcessRTPPayload(bufferWriter, payload, hdr.SequenceNumber, hdr.Timestamp, hdr.MarkerBit, out var isKeyFrame))
                    {
                        return true;
                    }
                }
