...
public class RTPChannel : IDisposable
{
    ...
    public bool IsDualMode
    {
        get
        {
            ...
            if (RtpSocket is { AddressFamily: AddressFamily.InterNetworkV6 } socket)
            {
    ...
    public RTPChannel(bool createControlSocket, IPAddress? bindAddress, int bindPort = 0, PortRange? rtpPortRange = null)
    {
        ...
        else if (createControlSocket && m_controlSocket is null)
        {
        ...
        RtpSocket = rtpSocket;
        Debug.Assert(RtpSocket is { });
        var rtpLocalEndPoint = RtpSocket.LocalEndPoint as IPEndPoint;
        Debug.Assert(rtpLocalEndPoint is { });
        RTPLocalEndPoint = rtpLocalEndPoint;
        Debug.Assert(RTPLocalEndPoint is { });
        RTPPort = RTPLocalEndPoint.Port;

        if (m_controlSocket is { })
        {
    ...
    public void StartRtpReceiver()
    {
        if (!m_rtpReceiverStarted)
        {
            ...
            logger.LogRtpChannelStarted(RtpSocket.LocalEndPoint);
    ...
    {
        if (!m_controlReceiverStarted && m_controlSocket is { })
        {
            m_controlReceiverStarted = true;

            Debug.Assert(m_controlSocket is { });
    ...
    public virtual SocketError Send(RTPChannelSocketsEnum sendOn, IPEndPoint dstEndPoint, ReadOnlyMemory<byte> buffer, IDisposable? memoryOwner = null)
    {
        ...
        else
        {
            try
            {
                Debug.Assert(RtpSocket is not null);
    ...
    protected virtual void OnRTPPacketReceived(UdpReceiver? receiver, int localPort, IPEndPoint? remoteEndPoint, ReadOnlyMemory<byte> packet)
    {
        ...
        if (!packet.IsEmpty && packet.Length >= RTPHeader.MIN_HEADER_LEN)
        {
            ...
            if (packet.Span[0] == 0x00 && packet.Span[1] == 0x17)
            {
                ...
                Debug.Assert(dataIndication is not null);
            ...
            if (packet.Span[0] is 0x00 or 0x01)
            {
                ...
                Debug.Assert(stunMessage is not null);
