    ...
    public void send(byte[] data, int offset = 0, int count = -1)
    {
        ...
        if (effectiveCount > _transport.maxMessageSize)
        {
            throw new ApplicationException(
                $"Data channel {label} was requested to send data of length {effectiveCount}  that exceeded the maximum allowed message size of {_transport.maxMessageSize}.");
        }
        else if (_transport.state != RTCSctpTransportState.Connected)
        {
            logger.LogWebRtcDataChannelSendFailed(_transport.state);
