...
public class RTCIceServer
{
    ...
    public static RTCIceServer Parse(ReadOnlySpan<char> iceServer)
    {
        int firstSep = iceServer.IndexOf(';');
        int secondSep = firstSep != -1 ? iceServer.Slice(firstSep + 1).IndexOf(';') : -1;
        ...
        if (firstSep == -1)
        {
            urls = iceServer.ToString();
        }
        else
        {
            urls = iceServer.Slice(0, firstSep).ToString();
            var rest = iceServer.Slice(firstSep + 1);
            if (secondSep == -1)
            {
                username = rest.ToString();
            }
            else
            {
                username = rest.Slice(0, secondSep).ToString();
                credential = rest.Slice(secondSep + 1).ToString();
            }
        }
...
public class RTCDtlsFingerprint
{
    ...
    public static bool TryParse(string str, out RTCDtlsFingerprint? fingerprint)
    {
        ...
        if (!DtlsUtils.IsHashSupported(algorithm.ToString()))
        {
            return false;
        }
