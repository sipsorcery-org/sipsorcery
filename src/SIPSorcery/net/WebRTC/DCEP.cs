...
public partial struct DataChannelOpenMessage : IByteSerializable
{
    ...
    public static DataChannelOpenMessage Parse(byte[] buffer, int posn) => Parse(buffer.AsSpan(posn));
    ...
    public static DataChannelOpenMessage Parse(ReadOnlySpan<byte> buffer)
    {
        ...
        {
            throw new SipSorceryException("The buffer did not contain the minimum number of bytes for a DCEP open message.");
        }
        ...
        var variableLength = labelLength + protocolLength;
        if (variableLength > buffer.Length - DCEP_OPEN_FIXED_PARAMETERS_LENGTH)
        {
            throw new SipSorceryException("The buffer was not big enough for the label and protocol lengths in the DCEP open message.");
        }

        if (labelLength > 0)
        {
            dcepOpen.Label = Encoding.UTF8.GetString(buffer.Slice(DCEP_OPEN_FIXED_PARAMETERS_LENGTH, labelLength));
        }

        if (protocolLength > 0)
        {
            dcepOpen.Protocol = Encoding.UTF8.GetString(buffer.Slice(DCEP_OPEN_FIXED_PARAMETERS_LENGTH + labelLength, protocolLength));
        }

        return dcepOpen;
    }

    public int GetLength() => GetByteCount();
    ...
    {
        using var bufferWriter = new ArrayPoolBufferWriter<byte>(buffer.Length);
        WriteTo(bufferWriter);
        bufferWriter.WrittenSpan.CopyTo(buffer.AsSpan(posn));
        return (ushort)bufferWriter.WrittenCount;
    }
