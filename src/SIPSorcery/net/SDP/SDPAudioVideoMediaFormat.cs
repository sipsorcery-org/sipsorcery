...
/// <summary>
/// Represents a single media format within a media announcement. Often the whole media format can be represented and
/// described by a single character, e.g. "0" without additional info represents standard "PCMU", "8" represents "PCMA"
/// etc. For other media types that have variable parameters additional attributes can be provided.
/// </summary>
/// <remarks>
/// This struct is designed to be immutable. If new information becomes available for a media format, such as when
/// parsing further into an SDP payload, a new media format should be created. TODO: With C#9 the struct could become a
/// "record" type.
/// </remarks>
public struct SDPAudioVideoMediaFormat
{
    ...
    /// media type, e.g "0" for "PCMU" etc, there is no guarantee that's the case. "0" can be used for any media format
    /// if there is a format attribute describing it. In the absence of a format attribute then it is required that it
    /// represents a standard media type.
    /// Note (rj2): FormatID MUST be string (not int), in case ID is 't38' and type is 'image' Note to above: The
    /// FormatID is always numeric for profile "RTP/AVP" and "RTP/SAVP", see
    /// https://tools.ietf.org/html/rfc4566#section-5.14 and section on "fmt": "If the [proto] sub-field is "RTP/AVP" or
    /// "RTP/SAVP" the [fmt] sub-fields contain RTP payload type numbers" In the case of T38 the format name is "t38"
    /// but the formatID must be set as a dynamic ID. <code> // Example // Note in this example "0" is representing a
    /// standard format so the format attribute is optional. m=audio 12228 RTP/AVP 0 101 // "0" and "101" are media
    /// format ID's. a=rtpmap:0 PCMU/8000 // "0" is the media format ID. a=rtpmap:101 telephone-event/8000 // "101" is
    /// the media format ID. a=fmtp:101 0-16 </code> <code> // t38 example from https://tools.ietf.org/html/rfc4612.
    ...
    /// <code> // Example a=rtpmap:0 PCMU/8000 a=rtpmap:101 telephone-event/8000 ← "101 telephone-event/8000" is the
    /// rtpmap properties. a=fmtp:101 0-16 </code>
    /// </summary>
    ...
    /// <code> // Example a=rtpmap:0 PCMU/8000 a=rtpmap:101 telephone-event/8000 a=fmtp:101 0-16 ← "101 0-16" is the
    /// fmtp attribute. </code>
    /// </summary>
    ...
    /// <summary>
    /// The standard name of the media format. <code> // Example a=rtpmap:0 PCMU/8000 ← "PCMU" is the media format name.
    /// a=rtpmap:101 telephone-event/8000 a=fmtp:101 0-16 </code>
    /// </summary>
    ...
    /// <summary>
    /// Creates a new SDP media format for a well known media type. Well known type are those that use ID's less than 96
    /// and don't require rtpmap or fmtp attributes.
    /// </summary>
    ...
    public bool IsAV1 => RtmapIs("AV1");

    public bool IsMJPEG => RtmapIs("JPEG");

    public bool isH265 => RtmapIs("H265");

    private bool RtmapIs(ReadOnlySpan<char> codec) => Rtpmap is not null && Rtpmap.AsSpan().TrimStart().StartsWith(codec, StringComparison.OrdinalIgnoreCase);
    ...
    /// <summary>
    /// Creates a new SDP media format for a dynamic media type. Dynamic media types are those that use ID's between 96
    /// and 127 inclusive and require an rtpmap attribute and optionally an fmtp attribute.
    /// </summary>
    ...
    /// <summary>
    /// Creates a new SDP media format from a Audio Format instance. The Audio Format contains the equivalent
    /// information to the SDP format object but has well defined audio properties separate from the SDP serialisation.
    /// </summary>
    ...
    /// <summary>
    /// Creates a new SDP media format from a Video Format instance. The Video Format contains the equivalent
    /// information to the SDP format object but has well defined video properties separate from the SDP serialisation.
    /// </summary>
    ...
    /// <summary>
    /// Creates a new media format based on an existing format but with a different ID. The typical case for this is
    /// during the SDP offer/answer exchange the dynamic format ID's for the equivalent type need to be adjusted by one
    /// party.
    /// </summary>
    ...
    /// <summary>
    /// For two formats to be a match only the codec and rtpmap parameters need to match. The fmtp parameter does not
    /// matter.
    /// </summary>
    ...
    /// <summary>
    /// Attempts to get the first compatible format between two lists without using LINQ or full iteration. This method
    /// is optimized for performance by returning immediately when the first compatible format is found.
    /// </summary>
    /// <param name="a">The first list to match the media formats for.</param>
    /// <param name="b">The second list to match the media formats for.</param>
    ...
    /// <param name="a">The first list to match the media formats for.</param>
    /// <param name="b">The second list to match the media formats for.</param>
    /// <param name="excludePayloadID">
    /// The payload ID to exclude from the search (typically RTP event payload ID).
    /// </param>
    /// <returns>
    /// The first compatible media format found that doesn't match the excluded payload ID, or Empty if no compatible
    /// formats exist.
    /// </returns>
    ...
    public static bool TryParseRtpmap(ReadOnlySpan<char> rtpmap, out string? name, out int clockRate, out int channels)
    {
        ...
        rtpmap.Split(fields, '/');
        var nameSpan = rtpmap[fields[0]].Trim();
        var clockRateSpan = rtpmap[fields[1]].Trim();
        var channelsSpan = rtpmap[fields[2]].Trim();

        if (!clockRateSpan.IsEmpty && !int.TryParse(clockRateSpan, out clockRate))
        {
            return false;
        }
    ...
    /// <summary>
    /// Attempts to get a common SDP media format that supports telephone events. If compatible an RTP event format will
    /// be returned that matches the local format with the remote format.
    /// </summary>
