...
#pragma warning disable SYSLIB0023
    ...
    private static readonly ILogger logger = LogFactory.CreateLogger(typeof(Crypto).FullName!);
    ...
    /// <summary>
    /// This version reads the whole file in at once. This is not great since it can consume a lot of memory if the file
    /// is large. However a buffered approach generates different hashes across different platforms.
    /// </summary>
    ...
    public static string GetHash(string filepath)
    {
        ...
        var overallHash = shaM.ComputeHash(fileBuffer);

        return Convert.ToBase64String(overallHash);
    }

    /// <summary>
    /// Used by methods wishing to perform a hash operation on a file. This method will perform a number of checks and
    /// if happy return a read only file stream.
    /// </summary>
    ...
    private static FileStream GetFileStream(string filepath)
    {
        ...
        {
            SysLoggingExtensions.LogCryptoHashNonExistentFile(logger, filepath);
        ...
        {
            inputStream.Close();
            SysLoggingExtensions.LogCryptoHashEmptyFile(logger, filepath);
    ...
    /// <summary>
    /// Returns the hash with each byte as an X2 string. This is useful for situations where the hash needs to only
    /// contain safe ASCII characters.
    /// </summary>
    ...
    /// Checks if the certificate is current and has a verifiable certificate issuer list. Should be set to false for
    ...
    public static X509Certificate2? LoadCertificate(StoreLocation storeLocation, string certificateSubject, bool checkValidity)
    {
        ...
        SysLoggingExtensions.LogCertStoreOpened(logger, store.Location);
        ...
        if (collection != null && collection.Count > 0)
        {
            ...
            logger.LogCertLoadedFromCurrentUserStore(serverCertificate.Subject, verifyCert);
            return serverCertificate;
        }
        else
        {
            logger.LogCertNotFoundInStore(certificateSubject, store.Location);
            return null;
        }
    }
}
