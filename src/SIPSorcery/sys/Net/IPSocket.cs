...
public static class IPSocket
{
    ...
    /// https://github.com/dotnet/corefx/blob/master/src/System.Net.Primitives/src/System/Net/IPEndPoint.cs. If/when
    /// that feature makes it into .NET Standard this method can be replaced.
    /// </summary>
    ...
    /// <summary>
    /// For IPv6 addresses with port the string format is of the form: [2a02:8084:6981:7880:54a9:d238:b2ee:ceb]:6060
    /// Without a port the form is: 2a02:8084:6981:7880:54a9:d238:b2ee:ceb
    /// </summary>
    ...
    public static int ParsePortFromSocket(string socket)
    {
        ...
        var lastColonPos = socket.LastIndexOf(':');
    ...
    public static bool IsIPAddress(string socket)
    {
        ...
        {
            return IPAddress.TryParse(socket, out var ipaddr);
        }
    }

    /// <summary>
    /// Checks the Contact SIP URI host and if it is recognised as a private address it is replaced with the socket the
    /// SIP message was received on.
    /// Private address space blocks RFC 1597. 10.0.0.0 - 10.255.255.255 172.16.0.0 - 172.31.255.255 192.168.0.0 -
    /// 192.168.255.255
    ///
    /// </summary>
    ...
    /// <summary>
    /// Check if <paramref name="endpointstring"/> contains a hostname or ip-address and ip-port accepts IPv4 and IPv6
    /// and IPv6 mapped IPv4 addresses return detected values in <paramref name="host"/> and <paramref name="port"/>
    /// adapted from: http://stackoverflow.com/questions/2727609/best-way-to-create-ipendpoint-from-string
    /// </summary>
    /// <remarks>
    /// rj2: I had the requirement of parsing an IPEndpoint with IPv6, v4 and hostnames and getting them as string and
    /// int
    /// </remarks>
    /// <param name="endpointstring">string to check</param>
    /// <param name="host">
    /// host-portion of <paramref name="endpointstring"/>, if host can be parsed as IPAddress, then
    /// <paramref name="host"/> is IPAddress.ToString
    /// </param>
    ...
    public static bool Parse(string endpointstring, out string host, out int port)
    {
        ...
        else if (values.Length > 2) //ipv6
        {
            ...
            {
                var ipaddressstring = string.Join(":", values.AsSpan(0, values.Length - 1));
