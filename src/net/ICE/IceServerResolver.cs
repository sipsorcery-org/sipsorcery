...
public class IceServerResolver
{
        ...
        if (iceServers is not null)
        {
            ...
            foreach (var cfg in iceServers)
            {
                ...
                while (!remaining.IsEmpty && count <= IceServer.MAXIMUM_ICE_SERVER_ID)
                {
                    ...
                    var server = new IceServer(stunUri, iceServerID++, cfg.username, cfg.credential, cfg.SslClientAuthenticationOptions);
    ...
    private void ScheduleDnsLookup(STUNUri key, IceServer server)
    {
        ...
        server.DnsResolutionTask = Task.Run(async () =>
        {
            ...
            var iceServersDictionary = iceServers.Count > 1 ? _iceServers.ToDictionary() : new Dictionary<STUNUri, IceServer>();
    ...
    public Task WaitForAllIceServersAsync(TimeSpan? timeout = null)
    {
        ...
        {
            if (server.DnsResolutionTask is not null)
            {
