    ...
    public partial class RtpIceChannel : RTPChannel
    {
        ...
        internal int RTO
        {
            get
            {
                ...
                {
                    return Math.Max(500, Ta * Candidates.Count(x => x.type is RTCIceCandidateType.srflx or RTCIceCandidateType.relay));
                }
                else
                {
                    lock (_checklistLock)
                    {
                        return Math.Max(500, Ta * (_checklist.Count(x => x.State is ChecklistEntryState.Waiting or ChecklistEntryState.InProgress)));
                    }
                }
            }
        }
        ...
        public event Action<int, IPEndPoint, ReadOnlyMemory<byte>> OnRtpIceDataReceived;
            ...
            base(false, bindAddress, bindPort, rtpPortRange)
        {
            ...
            Component = component;
            _iceServers = iceServers is null ? Array.Empty<RTCIceServer>() : iceServers.ToArray();
            _policy = policy;
            ...
            if (_iceServers is { Length: > 0 })
            {
                ...
                {
                    if (iceServer.Protocol == ProtocolType.Udp)
                    {
                        iceServerConnections[uri] = RtpConnection;
                    }
                    else if (iceServer.Protocol == ProtocolType.Tcp && iceServer.Uri.Scheme is STUNSchemesEnum.turn)
                    {
                        NetServices.CreateRtpSocket(
                            false, ProtocolType.Tcp, bindAddress, bindPort, rtpPortRange,
                            true, true, out var rtpTcpSocket, out _);
                    ...
                    else if (iceServer.Protocol == ProtocolType.Tcp && iceServer.Uri.Scheme is STUNSchemesEnum.turns)
                    {
                        NetServices.CreateRtpSocket(
                            false,
                            ProtocolType.Tcp,
        ...
        public void StartGathering()
        {
            if (!_closed && IceGatheringState == RTCIceGatheringState.@new)
            {
                ...
                if (_policy == RTCIceTransportPolicy.all)
                {
                    ...
                    foreach (var iceCandidateIPAddress in GetHostCandidateIPAddresses())
                    {
                        var iceCandidate = new RTCIceCandidate(init);
                        iceCandidate.SetAddressProperties(RTCIceProtocol.udp, iceCandidateIPAddress, (ushort)base.RTPPort, RTCIceCandidateType.host, null, 0);
                        ...
                        if (iceCandidate.component == RTCIceComponent.rtp && iceCandidate.sdpMLineIndex == SDP_MLINE_INDEX)
                        {
        ...
        /// <summary>
        /// Enumerates the local IP addresses eligible for use as ICE host candidates
        /// for each IP address that this host has except for:
        ...
        /// </summary>
        /// <returns>
        /// A sequence of <see cref="IPAddress"/> values representing the filtered local
        /// addresses that are valid for ICE host candidate generation.
        /// </returns>
        /// <remarks>
        ...
        /// </remarks>
        private IEnumerable<IPAddress> GetHostCandidateIPAddresses()
        {
            ...
            {
                var allAddresses = NetServices.GetLocalAddressesOnInterface(_bindAddress, _includeAllInterfaceAddresses);
        ...
        private void RefreshTurn(Object state)
        {
            try
            {
                         ...
                         REFRESH_PERMISSION_PERIOD)
                {
                    ...
                    logger.LogIceTurnPermissionsRequest(NominatedEntry.TurnPermissionsRequestSent, NominatedEntry.LocalCandidate.IceServer.Uri, NominatedEntry.RemoteCandidate.DestinationEndPoint, NominatedEntry.RequestTransactionID);
        ...
        private void CheckIceServers(object state)
        {
            ...
            if (Monitor.TryEnter(_iceServerResolver))
            {
                try
                {
                    ...
                    if (activeIceServerUri.Scheme is STUNSchemesEnum.turn or STUNSchemesEnum.turns && activeIceServer.RelayEndPoint != null ||
                        activeIceServerUri.Scheme is STUNSchemesEnum.stun && activeIceServer.ServerReflexiveEndPoint != null)
                    {
                    ...
                    if (activeIceServer.ServerEndPoint == null &&
                    ...
                    else if (activeIceServer.ServerEndPoint == null)
                    {
                    ...
                    else if (activeIceServer.ServerReflexiveEndPoint == null && activeIceServerUri.Scheme is STUNSchemesEnum.stun)
                    {
                        activeIceServer.Error = SendStunBindingRequest(_activeIceServer);
                    }
                    // Send TURN binding request.
                    else if (activeIceServer.ServerReflexiveEndPoint == null && activeIceServerUri.Scheme is STUNSchemesEnum.turn or STUNSchemesEnum.turns)
                    {
        ...
        private async Task UpdateChecklist(RTCIceCandidate localCandidate, RTCIceCandidate remoteCandidate)
        {
            ...
            try
            {
                ...
                if (remoteCandidate.DestinationEndPoint != null)
                {
                    ...
                    {
                        supportsIPv4 = RtpConnection.Socket.AddressFamily == AddressFamily.InterNetwork || base.IsDualMode;
                        supportsIPv6 = RtpConnection.Socket.AddressFamily == AddressFamily.InterNetworkV6 || base.IsDualMode;
                    }
                ...
                ChecklistEntry? FindMatchingChecklistEntry(ChecklistEntry targetEntry)
                {
                    ...
                    foreach (var x in _checklist)
                    {
                        if (x.LocalCandidate.type == targetEntry.LocalCandidate.type
                            ...
                            && x.RemoteCandidate.protocol == targetEntry.RemoteCandidate.protocol)
                        {
                            return x;
                        }
                    }
                        ...
                        if (_checklist.Count > 0)
                        {
                            ...
                            else
                            {
                                ...
                                foreach (var entry in _checklist)
                                {
                                    ...
                                    if (nextEntry == null && entry.State == ChecklistEntryState.Waiting)
                                    {
                                        nextEntry = entry;
                                    }
                                ...
                                if (retransmitEntry is not null)
                                {
        ...
        private void SendConnectivityCheck(ChecklistEntry candidatePair, bool setUseCandidate)
        {
            ...
            if (isRelayCheck && candidatePair.TurnPermissionsResponseAt == DateTime.MinValue)
            {
                ...
                {
                    logger.LogIceTurnPermissionsFailed(candidatePair.LocalCandidate.IceServer.Uri, candidatePair.TurnPermissionsRequestSent);
                ...
                else
                {
                    ...
                    logger.LogIceTurnPermissionsRequest(candidatePair.TurnPermissionsRequestSent, candidatePair.LocalCandidate.IceServer.Uri, candidatePair.RemoteCandidate.DestinationEndPoint, candidatePair.RequestTransactionID);
            ...
            else
            {
                if (candidatePair.LocalCandidate.type == RTCIceCandidateType.relay)
                {
                    ...
                    logger.LogIceConnectivityCheck(candidatePair.LocalCandidate, candidatePair.RemoteCandidate, base.RTPLocalEndPoint, relayServerEP, setUseCandidate);
                }
                else
                {
                    var remoteEndPoint = candidatePair.RemoteCandidate.DestinationEndPoint;
                    logger.LogIceRelayCheck(candidatePair.LocalCandidate, candidatePair.RemoteCandidate, base.RTPLocalEndPoint, remoteEndPoint, setUseCandidate);
                }
            ...
            else
            {
                ...
                else
                {
                        ...
                        (_remoteCandidates == null || !_remoteCandidates.Any(x => x.IsEquivalentEndPoint(RTCIceProtocol.udp, remoteEndPoint))))
                    {
                        ...
                        var entry = new ChecklistEntry(wasRelayed ? _relayChecklistCandidate : _localChecklistCandidate,
                    ...
                    else
                    {
                        ...
                        if (bindingRequest.Attributes.Any(x => x.AttributeType == STUNAttributeTypesEnum.UseCandidate))
                        {
                            if (IceConnectionState != RTCIceConnectionState.connected)
                            {
                                ...
                                logger.LogIceChecklistNominatedBinding(matchingChecklistEntry.RemoteCandidate.ToShortString());
                                SetNominatedEntry(matchingChecklistEntry);
                            }
                            else if (matchingChecklistEntry.RemoteCandidate.ToString() != NominatedEntry.RemoteCandidate.ToString())
                            {
                                ...
                                logger.LogIceNominatedNewCandidate(matchingChecklistEntry.RemoteCandidate.ToShortString());
                                SetNominatedEntry(matchingChecklistEntry);
                            }
                        }
...
#pragma warning restore CS8670 // Object or collection initializer implicitly dereferences possibly null member.
        ...
        protected override void OnRTPPacketReceived(SocketConnection? receiver, int localPort, IPEndPoint remoteEndPoint, ReadOnlyMemory<byte> packet)
        {
            if (!packet.IsEmpty)
            {
                ...
                if (packetSpan[0] == 0x00 && packetSpan[1] == 0x17)
                {
                    ...
                    using var activity = NetIceActivitySource.StartStunMessageReceivedActivity(dataIndication, remoteEndPoint, localPort, wasRelayed);

                    var foundData = false;
                    var foundPeerAddr = false;
                    foreach (var attr in dataIndication.Attributes)
                    {
                        switch (attr.AttributeType)
                        {
                            case STUNAttributeTypesEnum.Data:
                                packetSpan = attr.Value.Span;
                                foundData = true;
                                break;
                            case STUNAttributeTypesEnum.XORPeerAddress:
                                remoteEndPoint = (attr as STUNXORAddressAttribute)?.GetIPEndPoint() ?? remoteEndPoint;
                                foundPeerAddr = true;
                                break;
                        }

                        if (foundData && foundPeerAddr)
                        {
                            break;
                        }
                    }
                ...
                if (packetSpan[0] is 0x00 or 0x01)
                {
                    ...
                    using var activity = NetIceActivitySource.StartStunMessageReceivedActivity(stunMessage, remoteEndPoint, localPort, wasRelayed);
                    logger.LogReceivedStunResponse(remoteEndPoint, localPort, stunMessage);
                    _ = ProcessStunMessage(stunMessage, remoteEndPoint, wasRelayed);
                }
                else
                {
                    OnRtpIceDataReceived?.Invoke(localPort, remoteEndPoint, packet);
                }
            }
        }
        ...
        private SocketError SendStunMessage(STUNMessage stunMessage, IceServer iceServer)
        {
            ...
            var requestBytes = memoryOwner.Memory.Slice(0, bufferSize);
            stunMessage.WriteToBuffer(requestBytes.Span, messageIntegrityKeySpan, addFingerprint: true);
            return GetIceServerConection(iceServer).SendTo(iceServer.ServerEndPoint, requestBytes, memoryOwner);
        }
