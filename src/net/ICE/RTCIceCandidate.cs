...
public class RTCIceCandidate : IRTCIceCandidate, IEquatable<RTCIceCandidate>
{
    ...
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(candidateLine);
        ...
        if (uint.TryParse(NextField(ref span), out var priority))
        {
            candidate.priority = priority;
        }

        candidate.address = NextField(ref span).ToString();
        candidate.port = ushort.Parse(NextField(ref span));
        ...
        while (!span.IsEmpty)
        {
            ...
            {
                candidate.relatedPort = ushort.Parse(NextField(ref span));
            }
            else
            {
