...
/// </summary>
public partial class SctpErrorChunk : SctpChunk
{
    ...
    public override ushort GetByteCount(bool padded)
    {
        ...
        if (ErrorCauses is { } && ErrorCauses.Count > 0)
        {
    ...
    private void WriteBytesCore(Span<byte> buffer)
    {
        ...
        if (ErrorCauses is { Count: > 0 } errorCauses)
        {
            ...
            foreach (var cause in errorCauses)
            {
                var bytesWritten = cause.WriteBytes(buffer);
            ...
            foreach (var varParam in GetParameters(buffer.Slice(paramPosn, paramsBufferLength)))
            {
                ...
                switch (varParam.ParameterType)
                {
                    case (ushort)SctpErrorCauseCode.InvalidStreamIdentifier:
                        var streamID = (ushort)((varParam.ParameterValue is { }) ?
                    ...
                    case (ushort)SctpErrorCauseCode.StaleCookieError:
                        var staleness = (uint)((varParam.ParameterValue is { }) ?
                    ...
                    case (ushort)SctpErrorCauseCode.NoUserData:
                        uint tsn = (uint)((varParam.ParameterValue is { }) ?
                            BinaryPrimitives.ReadUInt32BigEndian(varParam.ParameterValue) : 0);
                    ...
                    case (ushort)SctpErrorCauseCode.UserInitiatedAbort:
                        var reason = (varParam.ParameterValue is { }) ?
                    ...
                    case (ushort)SctpErrorCauseCode.ProtocolViolation:
                        var info = (varParam.ParameterValue is { }) ?
                            Encoding.UTF8.GetString(varParam.ParameterValue) : null;
                ...
                if (stopProcessing)
                {
                    logger.LogSctpUnrecognisedParameter(varParam.ParameterType, SctpChunkType.ERROR);
                    break;
                }
            }
        }
