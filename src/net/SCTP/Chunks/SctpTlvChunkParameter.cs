...
public partial class SctpTlvChunkParameter
{
    ...
    public ushort ParseFirstWord(ReadOnlySpan<byte> buffer)
    {
        ...
        if (paramLen > 0 && buffer.Length < paramLen)
        {
            ...
            throw new SipSorceryException($"The SCTP chunk parameter buffer was too short. Required {paramLen} bytes but only {buffer.Length} available.");
        }
    ...
    public static SctpTlvChunkParameter ParseTlvParameter(ReadOnlySpan<byte> buffer)
    {
        ...
        {
            throw new SipSorceryException("Buffer did not contain the minimum of bytes for an SCTP TLV chunk parameter.");
        }
