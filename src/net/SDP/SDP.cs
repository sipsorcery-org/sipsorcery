...
namespace SIPSorcery.Net
{
    public partial class SDP
    {
        ...
        [GeneratedRegex(@"(?<type>\w+)\s+(?<port>\d+)(?:\/(?<portCount>\d+))?\s+(?<transport>\S+)\s*(?<formats>.*)$")]
        private static partial Regex MediaLineRegex();

        [GeneratedRegex(@"a=extmap:(?<id>\d+) (?<url>\S+)$")]
        private static partial Regex ExtensionMapRegex();

        [GeneratedRegex(@"a=rtpmap:(?<id>\d+)\s+(?<attribute>.*)$")]
        private static partial Regex FormatAttributeNumericRegex();

        [GeneratedRegex(@"a=rtpmap:(?<id>\S+)\s+(?<attribute>.*)$")]
        private static partial Regex FormatAttributeStringRegex();

        [GeneratedRegex(@"a=fmtp:(?<id>\d+)\s+(?<attribute>.*)$")]
        private static partial Regex FormatParametersNumericRegex();

        [GeneratedRegex(@"a=fmtp:(?<id>\S+)\s+(?<attribute>.*)$")]
        private static partial Regex FormatParametersStringRegex();
                    ...
                    foreach (string sdpLine in sdpLines)
                    {
                        ...
                        switch (sdpLineTrimmed)
                        {
                            ...
                            case var l when l.StartsWith("m="):
                                Match mediaMatch = MediaLineRegex().Match(
                            ...
                            case var l when l.StartsWith(SDPMediaAnnouncement.MEDIA_EXTENSION_MAP_ATTRIBUE_PREFIX):
                             ...
                             var formatAttributeMatch = ExtensionMapRegex().Match(sdpLineTrimmed);
                            ...
                            case var l when l.StartsWith(SDPMediaAnnouncement.MEDIA_FORMAT_ATTRIBUTE_PREFIX):
                                if (activeAnnouncement != null)
                                {
                                    if (activeAnnouncement.Media == SDPMediaTypesEnum.audio || activeAnnouncement.Media == SDPMediaTypesEnum.video || activeAnnouncement.Media == SDPMediaTypesEnum.text)
                                    {
                                        ...
                                        Match formatAttributeMatch = FormatAttributeNumericRegex().Match(sdpLineTrimmed);
                                        if (formatAttributeMatch.Success)
                                        {
                                    ...
                                    else
                                    {
                                        ...
                                        Match formatAttributeMatch = FormatAttributeStringRegex().Match(sdpLineTrimmed);
                                        if (formatAttributeMatch.Success)
                                        {
                                            string formatID = formatAttributeMatch.Result("${id}");
                                            string rtpmap = formatAttributeMatch.Result("${attribute}");
                            ...
                            case var l when l.StartsWith(SDPMediaAnnouncement.MEDIA_FORMAT_PARAMETERS_ATTRIBUE_PREFIX):
                                if (activeAnnouncement != null)
                                {
                                    if (activeAnnouncement.Media == SDPMediaTypesEnum.audio || activeAnnouncement.Media == SDPMediaTypesEnum.video || activeAnnouncement.Media == SDPMediaTypesEnum.text)
                                    {
                                        ...
                                        Match formatAttributeMatch = FormatParametersNumericRegex().Match(sdpLineTrimmed);
                                        if (formatAttributeMatch.Success)
                                        {
                                    ...
                                    else
                                    {
                                        ...
                                        Match formatAttributeMatch = FormatParametersStringRegex().Match(sdpLineTrimmed);
                                        if (formatAttributeMatch.Success)
                                        {
                                            string formatID = formatAttributeMatch.Result("${id}");
                                            string fmtp = formatAttributeMatch.Result("${attribute}");
