...
public class MediaStreamTrack
{
    ...
    MediaStreamStatusEnum streamStatus = MediaStreamStatusEnum.SendRecv) :
         this(SDPMediaTypesEnum.audio, false, ConvertAudioFormats(formats), streamStatus)
    { }
    ...
    MediaStreamStatusEnum streamStatus = MediaStreamStatusEnum.SendRecv) :
         this(SDPMediaTypesEnum.video, false, ConvertVideoFormats(formats), streamStatus)
    { }
    ...
    public MediaStreamTrack(params SDPWellKnownMediaFormatsEnum[] wellKnownAudioFormats)
        : this(ToAudioFormatList(wellKnownAudioFormats))
    { }
    ...
    {
        return Capabilities?.Exists(x => x.ID == payloadID) == true;
    }
    ...
    public SDPAudioVideoMediaFormat? GetFormatForPayloadID(int payloadID)
    {
        if (Capabilities is { })
        {
            foreach (var fmt in Capabilities)
            {
                if (fmt.ID == payloadID)
                {
                    return fmt;
    ...
    public ushort GetNextSeqNum()
    {
        ...
        do
        {
            if (++attempts > 10)
            {
                throw new SipSorceryException("GetNextSeqNum did not return an the next SeqNum due to concurrent updates from other threads within 10 attempts.");
            }
    ...
    private static List<SDPAudioVideoMediaFormat> ConvertAudioFormats(List<AudioFormat> formats)
    {
        var list = new List<SDPAudioVideoMediaFormat>(formats?.Count ?? 0);
        if (formats is { })
        {
            foreach (var f in formats)
            {
                list.Add(new SDPAudioVideoMediaFormat(f));
            }
        }
        return list;
    }

    private static List<SDPAudioVideoMediaFormat> ConvertVideoFormats(List<VideoFormat> formats)
    {
        var list = new List<SDPAudioVideoMediaFormat>(formats?.Count ?? 0);
        if (formats is { })
        {
    ...
    private static List<AudioFormat> ToAudioFormatList(SDPWellKnownMediaFormatsEnum[] wellKnownAudioFormats)
    {
        var list = new List<AudioFormat>(wellKnownAudioFormats?.Length ?? 0);
        if (wellKnownAudioFormats is { })
        {
            foreach (var wf in wellKnownAudioFormats)
            {
                list.Add(new AudioFormat(wf));
            }
        }
        return list;
    }
