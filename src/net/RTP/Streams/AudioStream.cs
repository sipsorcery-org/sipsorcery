...
namespace SIPSorcery.Net
{
    public class AudioStream : MediaStream
    {
        ...
        {
            if (encodedAudioFrame?.AudioFormat == null || encodedAudioFrame.AudioFormat.IsEmpty())
            {
        ...
        private SDPAudioVideoMediaFormat GetSendingFormat(AudioFormat audioFormat)
        {
            ...
            var capabilities = LocalTrack?.Capabilities ?? RemoteTrack?.Capabilities;
            if (capabilities == null)
            {
                throw new ApplicationException($"Cannot get the {MediaType} sending format, no capabilities available.");
            }
