...
public partial class RTCPTWCCFeedback : IByteSerializable
{
    ...
    public RTCPTWCCFeedback(ReadOnlySpan<byte> packet)
    {
        ...
        SenderSSRC = BinaryOperations.ReadUInt32BigEndianAndAdvance(ref packet);
        MediaSSRC = BinaryOperations.ReadUInt32BigEndianAndAdvance(ref packet);

        // Parse Base Sequence Number and Packet Status Count
        BaseSequenceNumber = BinaryOperations.ReadUInt16BigEndianAndAdvance(ref packet);
        PacketStatusCount = BinaryOperations.ReadUInt16BigEndianAndAdvance(ref packet);
    ...
    private List<TWCCPacketStatusType> ParseStatusChunks(ref ReadOnlySpan<byte> packet)
    {
        ...
        while (remainingStatuses > 0)
        {
            ...
            var chunk = BinaryOperations.ReadUInt16BigEndianAndAdvance(ref packet);
    ...
    private void WriteBytesCore(Span<byte> buffer)
    {
        ...
        BinaryOperations.WriteUInt32BigEndianAndAdvance(ref buffer, SenderSSRC);
        BinaryOperations.WriteUInt32BigEndianAndAdvance(ref buffer, MediaSSRC);

        // Write Base Sequence Number and Packet Status Count.
        BinaryOperations.WriteUInt16BigEndianAndAdvance(ref buffer, BaseSequenceNumber);
        BinaryOperations.WriteUInt16BigEndianAndAdvance(ref buffer, PacketStatusCount);

        // Build the 32-bit word for ReferenceTime and FeedbackPacketCount.
        BinaryOperations.WriteUInt32BigEndianAndAdvance(ref buffer, (ReferenceTime << 8) | FeedbackPacketCount);
        ...
        for (var i = 0; i < PacketStatuses.Count;)
        {
            ...
            if (runLength >= 2)
            {
                ...
                var chunk = (ushort)(statusBits << 12);
                chunk |= (ushort)(runLength & 0x0FFF);
                BinaryOperations.WriteUInt16BigEndianAndAdvance(ref buffer, chunk);
                i += runLength;
            }
            else
            {
